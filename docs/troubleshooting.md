...
- Port ladder (18770–18779 for prod, 18780–18789 for dev) is fully occupied
...
**Diagnosis**: Another phantom-api process is running, or the port ladder (default 18770–18779 for prod, 18780–18789 for dev) is occupied by something else.

**Fix**:
1. Find the occupying process: `lsof -i :18770` (or the relevant port)
...
### CLI or MCP talks to the wrong server ("invalid JSON from API", odd errors)

**Symptom**: the app works but `phantom …` or the MCP tools fail, or answer with something that is not Phantom's JSON.

**Diagnosis**: another process owns the default port (seen 2026-09-09: another vendor's local agent on 8768–8769, Phantom's default at the time — since moved to 18770), the API climbed the ladder, and a client assumed the default. `lsof -i :18770` names the squatter; `cat ~/Library/Application\ Support/phantom/api_url` is where the running API actually is.

**Fix**: nothing, since 1.1.0 — the API publishes its bound URL to that file and the CLI/MCP read it when `PHANTOM_API_URL` is unset. If the file is stale (the API crashed without cleaning up) the client's error names the file; relaunch the app and it is rewritten. `PHANTOM_API_URL` still overrides everything.
...
### A scan nobody asked for (or: who filled the database?)

**Symptom**: `phantom scans list` shows scans — a full home scan, repeated
`~/Library/Safari` scans — that nobody remembers starting.

**Diagnosis** (1.1.1, phantom-cnr.7): every `POST /scans` writes one line to
`~/Library/Logs/Phantom/phantom-api.log`:

```
INFO phantom_api::scans: scan requested id=… root=/Users/ted cross_volumes=false older_than=- verify_locks=false tool_estimates=false client=phantom-cli/1.1.1 (+verify)
INFO phantom_api::scans: scan finished id=… root=/Users/ted status=complete duration_s=118.4 files=4071459 dirs=609283 bytes=280400000000 rows=54012 estimated_db_bytes=41000000
INFO phantom_api::scans: retention: pruned 0 scans; estimated 0.9 GiB -> 0.9 GiB of a 2.0 GiB budget
```

`client` is the caller's User-Agent: `phantom-cli/<v>`, `phantom-mcp/<v>
(<MCP host>)`, `Phantom/<v>` for the app, `-` for a bare `curl`. `(+verify)`
means `phantom verify` / `verify_reclaim` was called without `--after` /
`afterScanId` and rescanned the plan's root itself — the usual source of a
"full scan nobody typed". Every prune and compaction logs its bytes on the
same stream, so the file's growth is attributable end to end.

**Fix**: pass `--after <scanId>` (CLI) or `afterScanId` (MCP) to verify
against a scan you already have; delete probe scans with
`phantom scans delete <id>` (that is logged too, with its client).

## Hostile filesystems (phantom-4x5)

What the scanner does when the filesystem misbehaves under it. The rule
throughout: an unreadable SUBTREE is counted (`errorCount`, the
`unreadablePaths` sample) and the scan completes honestly smaller; anything
that makes the totals describe nothing real fails the scan instead.

### The scan root is deleted, renamed or replaced mid-walk

The scan finishes `failed` with `failureReason: "scan root disappeared
during the walk: <path>"`. The walker records the root's identity (`dev`,
`ino`) when it starts and re-checks it after the parallel listing; a root
that is gone, is now a file, or was removed and recreated under the same
name all fail the check. Directories that had not been listed yet show up
as ENOENT in the unreadable sample — that is expected. Before v1.1 Phase 4
this case "completed" with whatever had been listed. Nothing is persisted
for a failed scan; rescan the new tree.

### Network mounts (SMB, NFS, AFP) stall or disappear

`std::fs` has no I/O timeout: a stalled network read blocks the worker
thread until the kernel's own mount timeout fires (minutes for SMB, longer
for hard NFS mounts). While it blocks, `cancel_scan` is accepted (202) but
takes effect only when that worker next checks the flag — i.e. after the
stalled call returns. `scan_status` keeps answering; the row stays
`running`. If the app is quit, the API child exits and the next start marks
the row `failed` ("interrupted: …"). Mitigations: by default a scan does
NOT descend into mount points below its root (`crossVolumes: false`, like
`du -x`), so a home scan never touches `/Volumes/*` or a mounted share;
scan a network share only by naming it as the root, and expect to wait out
its timeout if it drops. A share that disappears entirely is the root case
above once its directories return ENOENT/EIO — the scan fails rather than
completing with a stub.

### iCloud Drive and other cloud placeholders

A dataless placeholder (`flags: ["dataless"]`) is recorded with its full
`logicalSize` and ~0 `diskSize`; `diskSize` is THE size, so it counts what
is actually local. A file that downloads WHILE the scan runs is recorded
with whichever state the walker saw. The scan is a snapshot, not a
watcher: rescan after a large sync and use `diff_scans` / `phantom diff
--since` to see what landed. Evicting a file ("Remove Download") changes
`diskSize`, not `logicalSize`; the classifier's `cloudDataloaded` category
is excluded from the reclaim estimate for exactly this reason.

### Time Machine local snapshots and other APFS volumes

Freed space pinned by a local snapshot does not return until the snapshot
expires; `get_volume_status` / `phantom volume --snapshots` lists them and
`hidden.snapshotSuggestion` names the `tmutil thinlocalsnapshots` command
(Phantom never runs it). `usedBytes` from `statfs` is the whole APFS
container; `volumeUsedBytes` is the volume alone — see `hidden` on
`GET /volume` and the Gotchas entry in `CLAUDE.md`.
