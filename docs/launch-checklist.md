...
- [ ] Pre-flip audit of the public repo: `git ls-remote` shows ONLY
      HEAD/main/the tag on one commit under the noreply identity; release
      assets are exactly the DMG + appcast; zero PRs (automation minting
      `refs/pull/*` makes any pushed content permanent).
- [ ] Flip (`gh repo edit <owner>/<repo> --visibility public
      --accept-visibility-change-consequences`), then IMMEDIATELY
      `gh api -X PUT repos/<owner>/<repo>/private-vulnerability-reporting`
      — SECURITY.md promises that channel and it cannot be enabled while the
      repo is private. Confirm the appcast URL fetches unauthenticated (200).

## Release artifact

- [ ] `./scripts/release.sh <version>` green (alignment + verify + DMG + tag)
- [ ] DMG **notarized** (not SKIP_NOTARIZE) and `stapler validate` passes
      (on the DMG; the .app inside is not stapled as of 1.0.0 — phantom-ojj.1)
- [ ] Sparkle live-update test — a HARD GATE once the feed is public: a
      lower-version decoy installed in /Applications must discover, download,
      verify and install the published release (recipe in
      `docs/build-pipeline.md`, "Sparkle update channel"). An untested
      updater is worse than none.
