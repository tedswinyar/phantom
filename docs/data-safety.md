...
- Retention (`ScanStore::prune_retention`) bounds the live file's growth
  ...
  root**, then the newest **100 overall** (`Retention` in
  `rust/phantom-api/src/config.rs`; the defaults are a product decision —
  per root since 1.1 so one folder's history is never evicted by another's;
  operators may override with `PHANTOM_KEEP_SCANS_PER_ROOT` /
  `PHANTOM_KEEP_SCANS`, both positive, total ≥ per-root). A prune failure
