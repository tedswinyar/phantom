...
```bash
./scripts/start.sh &                       # dev-profile API (port 18780)
...
**Note**: `start.sh` runs the **dev** profile (port 18780). The CLI finds the
PROD server on its own (via `~/Library/Application Support/phantom/api_url`,
else 18770), so to talk to the dev server point it there explicitly:
`PHANTOM_API_URL=http://127.0.0.1:18780`.
