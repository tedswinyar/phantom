...
MIB=1048576
TREE="$WORK/haunt"
mkdir -p "$TREE/sub/deep" "$TREE/empty"
...
# 1.1.1 (phantom-cnr.10): directory rows obey the 1 MiB rule too. deep/ keeps
# its row ONLY because of this file (1 MiB + 4 KiB, so it sorts above the
# boundary case big.bin), and it sits at depth 2 so nesting is exercised
# level by level below; empty/ (0 B) is counted in the root's dirCount but
# has no row of its own — the honest 404 is checked in section 8.
perl -e 'print "\x0b" x (1024*1024 + 4096)' > "$TREE/sub/deep/nested.blob"
...
echo "$SCAN_JSON" | jq -e '.fileCount == 7 and .dirCount == 4 and .errorCount == 0' >/dev/null \
  || fail "scan totals wrong (want 7 files, 4 dirs): $SCAN_JSON"
...
echo "$FILES_HTTP" | jq -e '[.[].name] | sort == ["big.bin","cloud.dat","medium.log","nested.blob"]' >/dev/null \
...
http_get "$BASE/scans/$SCAN_ID/files?sort=path" \
  | jq -e '[.[].name] == ["big.bin","cloud.dat","nested.blob","medium.log"]' >/dev/null \
...
echo "$TYPES_HTTP" | jq -e '[.[].fileType] == ["dat","log","blob","bin","rs","txt"]' >/dev/null \
  || fail "type totals order/content wrong: $TYPES_HTTP"
TYPES_HUMAN="$("$BIN/phantom" types --scan "$SCAN_ID")"
# Line 1 is the scan header (1.1.1, phantom-cnr.4 — section 16 pins its shape);
# the totals start on line 2, largest first.
echo "$TYPES_HUMAN" | sed -n 1p | grep -qE "^scan [0-9a-f]{8} · $TREE · " \
  || fail "human types must open with the scan header: $(echo "$TYPES_HUMAN" | sed -n 1p)"
echo "$TYPES_HUMAN" | sed -n 2p | grep -qF '.dat' \
...
# 1.1.1: empty/ (0 B) has no row — it is counted, not listed. The gate must
# never pass "empty == empty": the exact set below is non-empty by
# construction and the parity checks above compared it byte-for-byte.
echo "$TREE_HTTP" | jq -e '[.[].name] == ["big.bin","cloud.dat","sub"]' >/dev/null \
  || fail "depth-1 tree wrong (path-ordered direct children; empty/ folded): $TREE_HTTP"
EMPTY_404="$(curl -s -G -H "x-api-key: $API_KEY" --data-urlencode "path=$TREE/empty" "$BASE/scans/$SCAN_ID/entry")"
echo "$EMPTY_404" | jq -e --arg root "$TREE" '
  (.error | test("not individually persisted")) and (.error | contains($root))
' >/dev/null || fail "entry on a folded directory must name the rule and the persisted ancestor: $EMPTY_404"
EMPTY_CODE="$(curl -s -o /dev/null -w '%{http_code}' -G -H "x-api-key: $API_KEY" --data-urlencode "path=$TREE/empty" "$BASE/scans/$SCAN_ID/tree")"
[ "$EMPTY_CODE" = 404 ] || fail "tree on a folded directory: want 404, got $EMPTY_CODE"
rc=0; "$BIN/phantom" tree --scan "$SCAN_ID" --path "$TREE/empty" >/dev/null 2>"$WORK/empty.err" || rc=$?
[ "$rc" = 3 ] || fail "CLI tree on a folded directory: want exit 3, got $rc"
grep -q 'not individually persisted' "$WORK/empty.err" \
  || fail "CLI must relay the honest 404: $(cat "$WORK/empty.err")"
...
# Nesting under the directory rule: deep/ is a row (its 1 MiB file), listed
# at depth 2 and descended at depth 3; the sets are exact, never empty.
DEEP_TREE="$(http_get "$BASE/scans/$SCAN_ID/tree?path=$TREE/sub" | jq -Sc .)"
echo "$DEEP_TREE" | jq -e '[.[].name] == ["deep","medium.log"]' >/dev/null \
  || fail "sub/ children wrong (want deep + medium.log; tiny.rs folded): $DEEP_TREE"
DEEP_CLI="$("$BIN/phantom" tree --scan "$SCAN_ID" --path "$TREE/sub" --depth 1 --json | jq -Sc .)"
[ "$DEEP_CLI" = "$DEEP_TREE" ] || fail "tree at sub/: CLI and HTTP disagree:
  cli:  $DEEP_CLI
  http: $DEEP_TREE"
"$BIN/phantom" tree --scan "$SCAN_ID" --depth 3 --json | jq -e '[.[].name] | index("nested.blob") != null' >/dev/null \
  || fail "--depth 3 must reach sub/deep/nested.blob"
"$BIN/phantom" tree --scan "$SCAN_ID" --depth 2 --json | jq -e '[.[].name] | index("nested.blob") == null' >/dev/null \
  || fail "--depth 2 must not reach sub/deep/nested.blob"
...
echo "$SCAN2_JSON" | jq -e '.fileCount == 7' >/dev/null \
...
# A bare target/ is not a hotspot, and the persistence layer proves it a
# second way (1.1.1): hotspot topPaths ALWAYS keep their row, so a decoy
# directory — a few hundred bytes, no detection file — has none, and the
# 404 names the rule and the ancestor that holds its bytes.
DECOY_404="$(curl -s -G -H "x-api-key: $API_KEY" --data-urlencode "path=$PROJ/decoys/target" "$BASE/scans/$PROJ_ID/entry")"
echo "$DECOY_404" | jq -e --arg proj "$PROJ" '
  (.error | test("not individually persisted")) and (.error | contains($proj))
' >/dev/null || fail "a bare target/ must have no row (not a topPath, under 1 MiB) and say so: $DECOY_404"
...
# Ordinary content that KEEPS a row under the 1.1.1 directory rule (src/ is
# 13 bytes and has none): a 1 MiB asset, dated like the sources so it does
# not move the project's activity.
mkdir -p "$OLDPROJ/assets"
perl -e 'print "\x0c" x (1024*1024)' > "$OLDPROJ/assets/photo.bin"
OLDSTAMP="$(perl -MPOSIX -e 'print strftime("%Y%m%d%H%M", localtime(time - 200*86400))')"
touch -t "$OLDSTAMP" "$OLDPROJ/Cargo.toml" "$OLDPROJ/Cargo.lock" "$OLDPROJ/src/main.rs" "$OLDPROJ/assets/photo.bin"
...
"$BIN/phantom" explain "$OLDPROJ/assets" --scan "$INS_ID" --json | jq -e '.hotspot == null and .category == null and (.summary | test("not a hotspot"))' >/dev/null \
  || fail "explain of ordinary content must say so"
# A folded directory (1.1.1): src/ has no row; the 404 travels through the
# CLI (exit 3) naming the rule and the project root that holds its bytes.
rc=0; "$BIN/phantom" explain "$OLDPROJ/src" --scan "$INS_ID" --json >/dev/null 2>"$WORK/explain.err" || rc=$?
[ "$rc" = 3 ] || fail "explain of a folded directory: want exit 3, got $rc"
grep -q 'not individually persisted' "$WORK/explain.err" && grep -qF "\"$OLDPROJ\"" "$WORK/explain.err" \
  || fail "explain of a folded directory must name the rule and the ancestor: $(cat "$WORK/explain.err")"
...
# ---------------------------------------------------------------------------
# 16. Default-scan honesty (v1.1.1, phantom-cnr.4). Every human view opens
# with a header naming the scan it read; a DEFAULTED pick warns when a larger
# scan of another root is under a day old; the positional id is an alias for
# --scan; the CLI header and the MCP note agree byte-for-byte (the trailing
# age is stripped — two surfaces render seconds apart). The 2026-09-16 shape:
# a big scan, then a small probe scan that silently became the default.
# CLI output is captured to variables before any pipe (the SIGPIPE gotcha).
# ---------------------------------------------------------------------------
BIGROOT="$WORK/bigroot"; mkdir -p "$BIGROOT"
perl -e 'print "\x0c" x (16*1024*1024)' > "$BIGROOT/sixteen.bin"   # 16 MiB — larger than every fixture above
BIG_ID="$("$BIN/phantom" scan "$BIGROOT" --json | jq -r .id)"
SMALLROOT="$TREE/sub"                                               # ~3 MiB, already on disk
SMALL_ID="$("$BIN/phantom" scan "$SMALLROOT" --json | jq -r .id)"
short_id() { printf '%s' "$1" | tr -d - | cut -c1-8; }
strip_age() { sed 's/ · [^·]*$//'; }
first_line() { printf '%s\n' "$1" | sed -n 1p; }

# The header line opens every human view and names the scan.
HOT_OUT="$("$BIN/phantom" hotspots 2>"$WORK/hot-default.err")"
HDR_DEFAULT="$(first_line "$HOT_OUT")"
case "$HDR_DEFAULT" in
  "scan $(short_id "$SMALL_ID") · $SMALLROOT · "*" files · "*) : ;;
  *) fail "defaulted hotspots must open with the scan header: $HDR_DEFAULT" ;;
esac
for cmd in top tree types plan stale; do
  OUT="$("$BIN/phantom" $cmd --scan "$SMALL_ID" 2>/dev/null)"
  [ "$(first_line "$OUT" | strip_age)" = "$(printf '%s\n' "$HDR_DEFAULT" | strip_age)" ] \
    || fail "phantom $cmd must open with the same header line: $(first_line "$OUT")"
done
OUT="$("$BIN/phantom" explain "$SMALLROOT/medium.log" --scan "$SMALL_ID")"
[ "$(first_line "$OUT" | strip_age)" = "$(printf '%s\n' "$HDR_DEFAULT" | strip_age)" ] \
  || fail "phantom explain must open with the header line: $(first_line "$OUT")"

# The warning: the default is the small scan; the 16 MiB scan is larger and minutes old.
grep -qF "a larger recent scan exists: $BIGROOT" "$WORK/hot-default.err" \
  || fail "defaulted hotspots must warn about the larger recent scan on stderr: $(cat "$WORK/hot-default.err")"
grep -qF "pass --scan $BIG_ID" "$WORK/hot-default.err" || fail "the warning must name the remedy"
# --json: stdout stays the API body (no header, no note); the warning still reaches stderr.
HOT_JSON="$("$BIN/phantom" hotspots --json 2>"$WORK/hot-json.err")"
echo "$HOT_JSON" | jq -e 'has("groups") and (has("note") | not)' >/dev/null \
  || fail "hotspots --json must stay the API body: $HOT_JSON"
grep -qF 'a larger recent scan exists' "$WORK/hot-json.err" || fail "the warning must reach stderr under --json too"
# An explicit id never warns.
"$BIN/phantom" hotspots --scan "$SMALL_ID" >/dev/null 2>"$WORK/hot-explicit.err"
[ ! -s "$WORK/hot-explicit.err" ] || fail "an explicit --scan must not warn: $(cat "$WORK/hot-explicit.err")"

# MCP: a defaulted call carries a note — first line the CLI header (age stripped),
# second line the CLI warning with the MCP remedy. An explicit scanId carries no note.
MCP_DEFAULT="$(mcp_result get_hotspots '{}')"
MCP_NOTE="$(echo "$MCP_DEFAULT" | jq -r .note)"
MCP_NOTE_HEAD="$(first_line "$MCP_NOTE" | sed 's/^scanId omitted; read //')"
[ "$(printf '%s\n' "$MCP_NOTE_HEAD" | strip_age)" = "$(printf '%s\n' "$HDR_DEFAULT" | strip_age)" ] \
  || fail "MCP note header and CLI header must agree byte-for-byte:
  mcp: $MCP_NOTE_HEAD
  cli: $HDR_DEFAULT"
CLI_WARN="$(sed -n 's/^phantom: //p' "$WORK/hot-default.err" | sed 's/ — pass --scan .*$//')"
MCP_WARN="$(printf '%s\n' "$MCP_NOTE" | sed -n 2p | sed 's/ — pass scanId .*$//')"
[ -n "$CLI_WARN" ] && [ "$MCP_WARN" = "$CLI_WARN" ] || fail "MCP note warning and CLI stderr warning must agree:
  mcp: $MCP_WARN
  cli: $CLI_WARN"
echo "$MCP_DEFAULT" | jq -e --arg id "$BIG_ID" '.note | contains("pass scanId \"" + $id + "\"")' >/dev/null \
  || fail "the MCP remedy must name scanId: $MCP_NOTE"
MCP_EXPLICIT="$(mcp_result get_hotspots "{\"scanId\":\"$SMALL_ID\"}")"
echo "$MCP_EXPLICIT" | jq -e 'has("note") | not' >/dev/null || fail "an explicit scanId must carry no note"
[ "$(echo "$MCP_EXPLICIT" | jq -Sc .)" = "$(http_get "$BASE/scans/$SMALL_ID/hotspots" | jq -Sc .)" ] \
  || fail "explicit get_hotspots must still equal the HTTP body byte-for-byte"
# get_space_by_type is a bare array: no note to carry, and still the verbatim body.
[ "$(mcp_result get_space_by_type '{}' | jq -Sc .)" = "$(http_get "$BASE/scans/$SMALL_ID/types" | jq -Sc .)" ] \
  || fail "get_space_by_type stays the bare-array body when defaulted"

# Positional id == --scan, byte for byte; both at once is a usage error (2).
POS_OUT="$("$BIN/phantom" hotspots "$BIG_ID")"
FLAG_OUT="$("$BIN/phantom" hotspots --scan "$BIG_ID")"
[ "$(printf '%s\n' "$POS_OUT" | strip_age)" = "$(printf '%s\n' "$FLAG_OUT" | strip_age)" ] \
  || fail "positional scan id must equal --scan"
rc=0; "$BIN/phantom" hotspots "$BIG_ID" --scan "$BIG_ID" >/dev/null 2>&1 || rc=$?
[ "$rc" = 2 ] || fail "positional id AND --scan must be a usage error (2), got $rc"

# Once the big root is the newest scan there is nothing to warn about, on either surface.
BIG2_ID="$("$BIN/phantom" scan "$BIGROOT" --json | jq -r .id)"
TYPES_OUT="$("$BIN/phantom" types 2>"$WORK/types-default.err")"
case "$(first_line "$TYPES_OUT")" in
  "scan $(short_id "$BIG2_ID") · $BIGROOT · "*) : ;;
  *) fail "the newest scan is the default: $(first_line "$TYPES_OUT")" ;;
esac
[ ! -s "$WORK/types-default.err" ] \
  || fail "no warning when the default is the largest recent scan: $(cat "$WORK/types-default.err")"
QUIET_NOTE="$(mcp_result get_hotspots '{}' | jq -r .note)"
if printf '%s\n' "$QUIET_NOTE" | grep -qF 'a larger recent scan'; then
  fail "MCP note must not warn when the default is the largest: $QUIET_NOTE"
fi
[ "$(printf '%s\n' "$QUIET_NOTE" | wc -l | tr -d ' ')" = 1 ] || fail "a quiet note is the header alone: $QUIET_NOTE"
echo "e2e: default-scan header, warning and positional id hold across CLI and MCP"

# ---------------------------------------------------------------------------
# 17. Scan provenance (v1.1.1, phantom-cnr.7): every POST /scans leaves ONE
# attributable line in the API log — root, options and the caller's
# User-Agent — so a scan nobody remembers asking for is traced to the binary
# that asked, not to a transcript grep. The CLI, the MCP server and verify's
# implicit rescan each name themselves. $WORK/api.err is the API's stderr,
# the stream ~/Library/Logs/Phantom/phantom-api.log holds in production.
# ---------------------------------------------------------------------------
REQUESTED="$(grep 'scan requested' "$WORK/api.err" || true)"
SCANS_MADE="$(http_get "$BASE/scans" | jq 'length')"
[ "$(printf '%s\n' "$REQUESTED" | grep -c 'scan requested')" -ge "$SCANS_MADE" ] \
  || fail "one 'scan requested' line per POST /scans: $SCANS_MADE scans exist, log has $(printf '%s\n' "$REQUESTED" | grep -c 'scan requested')"
printf '%s\n' "$REQUESTED" | grep -qF 'client=phantom-cli/' || fail "the CLI must name itself in the API log: $REQUESTED"
printf '%s\n' "$REQUESTED" | grep -qF 'client=phantom-mcp/' || fail "the MCP server must name itself in the API log: $REQUESTED"
printf '%s\n' "$REQUESTED" | grep -qF "root=$TREE cross_volumes=" || fail "the start line must name the root and options: $REQUESTED"
grep -qF 'scan finished' "$WORK/api.err" || fail "every completion must leave a 'scan finished' line"
grep 'scan finished' "$WORK/api.err" | grep -qF 'status=complete' || fail "the completion line carries the status"
grep 'scan finished' "$WORK/api.err" | grep -qE 'rows=[0-9]+ estimated_db_bytes=[0-9]+' || fail "the completion line carries rows and estimated bytes"
grep -qF 'retention: pruned' "$WORK/api.err" || fail "every completion must log the retention pass with its bytes"
# verify without --after rescans the plan's root: the log says who AND why,
# and the terminal names the scan and the flag that would have avoided it.
# (The verdict itself is not the point here — the tree has moved on since
# section 14 — so the exit code is not asserted; the rescan is.)
rc=0; "$BIN/phantom" verify "$PLAN_ID" >/dev/null 2>"$WORK/verify-rescan.err" || rc=$?
[ "$rc" = 0 ] || [ "$rc" = 1 ] || fail "verify by rescan died ($rc): $(cat "$WORK/verify-rescan.err")"
grep -qF "rescanning $PLANROOT as scan " "$WORK/verify-rescan.err" || fail "verify must name the rescan it started: $(cat "$WORK/verify-rescan.err")"
grep -qF -- '--after <scanId>' "$WORK/verify-rescan.err" || fail "verify must point at --after: $(cat "$WORK/verify-rescan.err")"
grep 'scan requested' "$WORK/api.err" | grep -F "root=$PLANROOT " | grep -F 'client=phantom-cli/' | grep -qF '(+verify)' \
  || fail "the CLI's implicit rescan must carry (+verify) in its User-Agent: $(grep 'scan requested' "$WORK/api.err" | tail -1)"
# Section 14's verify_reclaim without afterScanId did the same from the MCP side.
grep 'scan requested' "$WORK/api.err" | grep -F 'client=phantom-mcp/' | grep -qF '(+verify)' \
  || fail "the MCP server's implicit rescan must carry (+verify) in its User-Agent"
echo "e2e: scan provenance — CLI, MCP and verify's rescan are attributable in the API log"
