...
public enum Size {
    ...
    /// Sidebar sparkline (Phase 4 history): small enough to sit in a row.
    public static let sparklineWidth: CGFloat = 44
    public static let sparklineHeight: CGFloat = 14
    /// The History pane's chart height and its line weights.
    public static let chartHeight: CGFloat = 160
    public static let chartLineWidth: CGFloat = 1.5
    public static let sparklineLineWidth: CGFloat = 1
    ...
    /// Settings window: a fixed width is the Mac convention (System
    /// Settings, Xcode); height follows the selected tab's content.
    public static let settingsWidth: CGFloat = 460
    public static let settingsHeight: CGFloat = 240
...
public enum TreemapStyle {
    ...
    /// Outline on tiles that are new or grew since the previous scan of the
    /// root (Phase 4 delta highlighting) — thinner than selection, so the
    /// two read differently when they coincide.
    public static let deltaLineWidth: CGFloat = 2
...
public enum Palette {
    ...
    public static func categoryColor(for category: String?) -> Color {
        switch category {
        case "staleProjectArtifact", "regenerableArtifact", "cache", "toolManagedCache", "modelCache":
            return success
