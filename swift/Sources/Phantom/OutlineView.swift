...
struct FolderOutlineView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        // Phase 4: what moved since the previous scan of this root — the
        // server's top movements only, so an empty cell means "not among
        // the biggest changes", not "unchanged".
        outline.addTableColumn(column("change", "Since last scan", width: 120))
    ...
    func updateNSView(_ scrollView: NSScrollView, context: Context) {
        ...
        } else if deltaID != coordinator.lastDeltaID {
            // The delta arrives after the first children do; refresh the
            // change column without disturbing the disclosure state.
            coordinator.lastDeltaID = deltaID
            coordinator.reloadPreservingExpansion()
        }
        coordinator.lastDeltaID = deltaID
    ...
    final class Coordinator: NSObject, NSOutlineViewDataSource, NSOutlineViewDelegate, NSMenuDelegate {
        ...
        func outlineView(
            ...
            case "change":
                guard node.entry != nil else { return nil }
                switch model.deltaMark(for: node.path) {
                case .new(let bytes):
                    return sizeCell(text: "new  +\(Format.size(bytes))", outlineView, id: "changeCell", secondary: false)
                case .grown(let bytes):
                    return sizeCell(text: "+\(Format.size(bytes))", outlineView, id: "changeCell", secondary: false)
                case nil:
                    return sizeCell(text: "", outlineView, id: "changeCell", secondary: true)
                }
            default:
                return nil
            }
