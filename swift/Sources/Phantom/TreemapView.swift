...
struct TreemapView: View {
    ...
    private func draw(layout: TreemapLayout, in context: GraphicsContext, size: CGSize) {
        ...
        for (rect, scaled) in visible {
            ...
            // Phase 4: tiles that are new or grew since the previous scan of
            // this root wear a warning outline — the server's top movements,
            // not a recomputation.
            if !rect.residual, model.deltaMark(for: rect.path) != nil {
                context.stroke(
                    Path(scaled), with: .color(Palette.warning),
                    lineWidth: TreemapStyle.deltaLineWidth
                )
            }
            if isSelected {
                context.stroke(
