                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        Grid(
                            ...
                            if let privateSize = entry.privateSize, let sharedSize = entry.sharedSize,
                               sharedSize > 0 {
                                // v1.1: what deleting this would ACTUALLY free
                                // — shown only when it differs from the size,
                                // i.e. when hard links, clones or snapshots
                                // outside it pin part of the bytes.
                                DetailRow(label: "Deleting Frees", value: Format.size(privateSize))
                                DetailRow(label: "Shared", value: Format.size(sharedSize))
                            }
                            if let flags = entry.flags, !flags.isEmpty {
                                // Raw wire strings: a new server-side flag
                                // renders as itself rather than failing.
                                DetailRow(label: "Flags", value: flags.joined(separator: ", "))
                            }
