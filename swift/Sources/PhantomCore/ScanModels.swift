...
public struct Scan: Codable, Identifiable, Equatable, Sendable {
    ...
    /// What deleting the whole tree would actually free (the root's
    /// `privateSize`); ≤ `totalDiskSize`. `nil` == not recorded (rows
    /// persisted before schema v5).
    public var totalPrivateSize: UInt64?
    /// Bytes under the root that other paths, clones or snapshots pin.
    /// `nil` == not recorded (pre-v5).
    public var totalSharedSize: UInt64?
    /// Why a failed scan failed: the walker's error, or the server's
    /// cold-start marker ("interrupted: …") for a scan it was running when
    /// it stopped. `nil` for every other status and for pre-1.1 bodies.
    ...
    public init(
        ...
        totalPrivateSize: UInt64? = nil, totalSharedSize: UInt64? = nil,
        ...
        self.totalSharedSize = totalSharedSize
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case totalPrivateSize, totalSharedSize, failureReason
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let totalSharedSize { try c.encode(totalSharedSize, forKey: .totalSharedSize) } else { try c.encodeNil(forKey: .totalSharedSize) }
        if let failureReason { try c.encode(failureReason, forKey: .failureReason) } else { try c.encodeNil(forKey: .failureReason) }
    }
}
...
public struct ScanEntry: Codable, Identifiable, Equatable, Sendable {
    ...
    /// What deleting this path would ACTUALLY free. Files: 0 when other hard
    /// links or an APFS clone share every block, the rewritten blocks of a
    /// modified clone, `diskSize` for an ordinary file. Directory rows: what
    /// deleting the subtree frees — a hardlink/clone group counts only if
    /// EVERY reference to it is inside. `nil` == not recorded (pre-v5 rows).
    public let privateSize: UInt64?
    /// `diskSize − privateSize` for files; for directories the allocation
    /// of every group that is also referenced outside the subtree, plus
    /// the shared part of partially-cloned files. `nil` == not recorded.
    public let sharedSize: UInt64?
    /// APFS clone-group id when this file shares ALL its blocks with at
    /// least one other file (every member carries the same id; the group
    /// counts once, like a hardlinked inode). `nil` == not a pure clone,
    /// or not recorded. Directories: always nil.
    public let cloneId: UInt64?
    /// Filesystem facts as wire strings (`dataless`, `purgeable`, `sparse`,
    /// `mayShareBlocks`, `sharesAllBlocks`, `firmlink`, `mountPoint`); kept
    /// raw so a new server-side flag degrades to "unknown", not a decode
    /// failure. `[]` when none; `nil` == not recorded (pre-v5 rows).
    ...
    /// True when the wire `flags` array names this fact.
    public func hasFlag(_ name: String) -> Bool { flags?.contains(name) ?? false }
    ...
    public init(
        ...
        privateSize: UInt64? = nil, sharedSize: UInt64? = nil,
        cloneId: UInt64? = nil, flags: [String]? = nil
    ) {
        self.path = path
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case fileCount, dirCount, privateSize, sharedSize, cloneId, flags
    }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if let privateSize { try c.encode(privateSize, forKey: .privateSize) } else { try c.encodeNil(forKey: .privateSize) }
        if let sharedSize { try c.encode(sharedSize, forKey: .sharedSize) } else { try c.encodeNil(forKey: .sharedSize) }
        if let cloneId { try c.encode(cloneId, forKey: .cloneId) } else { try c.encodeNil(forKey: .cloneId) }
        if let flags { try c.encode(flags, forKey: .flags) } else { try c.encodeNil(forKey: .flags) }
    }
}
