...
public final class ScansModel {
    ...
    /// Growth over time (1.1.0 Phase 4): the selected scan's root across
    /// its completed scans, with the forecast. Nil until loaded; a 404
    /// (no completed scan of the root — only while the selected scan is
    /// itself not complete) is `growthUnavailable`, not an error.
    public private(set) var growth: Growth?
    public private(set) var growthUnavailable = false
    /// The breakdown the History view asked for; reloads on change.
    public var growthGroupBy: GrowthGroupBy = .total

    /// "New since last scan" (Phase 4, phantom-mkn.23.1): the diff from the
    /// previous complete scan of the same root to the selected one. Nil when
    /// the selected scan is the root's first, or while loading. Its `grown`
    /// list is the server's top movements (≥ 1 MiB, at most 20 directories)
    /// — the highlight marks THOSE, it does not recompute the tree.
    public private(set) var delta: ScanDiff?
    /// The scan `delta` reads from; nil when there is none.
    public private(set) var deltaBaseline: Scan?

    /// Compare-any-two (Phase 4): the other complete scan of the root the
    /// History pane is comparing the selection against, and the result.
    public var compareWithID: UUID?
    public private(set) var comparison: ScanDiff?

    /// Growth notification (v1.2 Phase 2, phantom-adq.2). The trigger —
    /// "a SCHEDULED scan just completed" — arrives with Phase 1's
    /// `startedBy`; until then nothing calls `notifyGrowthIfWarranted`
    /// except tests. Off by default until Settings grows the switch.
    public var growthAlertsEnabled = false
    public var growthAlertThresholds: GrowthAlertThresholds = .standard
    /// Injected so tests record deliveries and the store is not the real
    /// UserDefaults.
    public var growthAlertDeliverer: GrowthAlertDelivering?
    public var notifiedScans: NotifiedScanStore = DefaultsNotifiedScanStore()

    /// How a path moved since the previous scan of its root.
    public enum DeltaMark: Equatable, Sendable {
        /// Absent from the previous scan.
        case new(bytes: UInt64)
        /// Present before, bigger now.
        case grown(bytes: UInt64)
    }
    ...
    /// Builds the client for a (re)announced server URL. Injected so the
    /// auto-restart path is testable with a mock; the default resolves the
    /// key the same way connect() always did.
    private let clientFactory: @Sendable (URL) -> APIClientProtocol
    ...
    public init(
        ...
        self.clientFactory = Self.defaultClientFactory
    }

    /// The production client for an announced URL: key from the env or the
    /// key file, like every other Phantom client.
    private static let defaultClientFactory: @Sendable (URL) -> APIClientProtocol = { url in
        APIClient(
            baseURL: url,
    ...
    init(
        ...
        fdaProbe: FileAccessProbing = FileSystemAccessProbe(),
        clientFactory: (@Sendable (URL) -> APIClientProtocol)? = nil
    ) {
        self.client = client
    ...
    public func connect() async {
        ...
        // A Retry the user chose is a fresh start for the crash budget too.
        APIServerManager.shared.resetRestartBudget()
        // Bounded auto-restart (phantom-rrw): when the child dies
        // mid-session the manager relaunches it and reports here; the port
        // may differ, so the client is rebuilt from the new URL.
        APIServerManager.shared.onRestart = { [weak self] result in
            Task { @MainActor in await self?.serverRestarted(result) }
        }
        do {
            let url = try await APIServerManager.shared.startIfNeeded()
            self.client = clientFactory(url)
        } catch {
    ...
    /// The manager's verdict after a mid-session crash: back on a (possibly
    /// new) URL → re-point the client and reload; budget spent or relaunch
    /// failed → the failed screen, whose Retry starts a fresh budget.
    public func serverRestarted(_ result: Result<URL, APIServerManager.RestartOutcome>) async {
        switch result {
        case .success(let url):
            client = clientFactory(url)
            connectionState = .connecting
            for id in Array(pollTasks.keys) { stopPolling(id: id) }
            await refresh()
        case .failure(.crashLoop(let n)):
            connectionState = .failed(
                "phantom-api stopped \(n) times within a minute and was not restarted again. See \(APIServerManager.logFileURL.path), then Retry."
            )
        case .failure(.relaunchFailed(let why)):
            connectionState = .failed(
                "phantom-api stopped and could not be restarted (\(why)). See \(APIServerManager.logFileURL.path), then Retry."
            )
        }
    }
    ...
    /// Remove a scan and its recorded results from Phantom's database
    /// (DELETE /scans/{id}). Files on disk are never touched — this forgets
    /// an analysis, nothing more. The API refuses a running scan with 409
    /// (cancel first); that and any other failure surface as `lastError`
    /// with the row left in place. When the removed scan was selected, the
    /// selection moves to its neighbour (the row below, else the row above)
    /// the way Finder and Mail do, rather than dropping to no selection.
    public func remove(id: UUID) async {
        guard let client else { return }
        do {
            try await client.deleteScan(id: id)
        } catch {
            lastError = error.localizedDescription
            return
        }
        stopPolling(id: id)
        let wasSelected = selectedScanID == id
        let neighbour = wasSelected ? Self.neighbourID(of: id, in: scans) : nil
        scans.removeAll { $0.id == id }
        lastError = nil
        if wasSelected {
            await select(scanID: neighbour)
        }
    }

    /// The row a removal should hand selection to: the one after `id`,
    /// else the one before, else nil (the list is emptying).
    static func neighbourID(of id: UUID, in scans: [Scan]) -> UUID? {
        guard let idx = scans.firstIndex(where: { $0.id == id }) else { return nil }
        if idx + 1 < scans.count { return scans[idx + 1].id }
        if idx > 0 { return scans[idx - 1].id }
        return nil
    }
    ...
    /// "Copy as script" (v1.1): build a plan for the selected scan at
    /// `maxTier` (safe by default) and fetch it as the dry-run shell script.
    /// Returns nil (and sets `lastError`) on failure. The caller puts the
    /// text on the clipboard — this model never runs anything.
    public func reclaimScript(maxTier: String = "safe") async -> String? {
        guard let client, let scan = selectedScan else { return nil }
        do {
            let plan = try await client.createPlan(scanID: scan.id, maxTier: maxTier)
            let script = try await client.planScript(planID: plan.planId)
            lastError = nil
            return script
        } catch {
            lastError = error.localizedDescription
    ...
    /// Load the growth series for the selected scan's root at the current
    /// `growthGroupBy`. A 404 means no completed scan of the root exists
    /// yet — expected for a cancelled/failed selection — not an error.
    public func loadGrowth() async {
        guard let client, let scan = selectedScan else { return }
        do {
            growth = try await client.getGrowth(root: scan.rootPath, groupBy: growthGroupBy.rawValue)
            growthUnavailable = false
            lastError = nil
    ...
    /// Load the previous-scan delta for the selected COMPLETE scan. No
    /// previous complete scan of the root → nothing to show, not an error.
    public func loadDelta() async {
        guard let client, let scan = selectedScan, scan.status == .complete else { return }
        guard let previous = Self.previousCompleteScan(before: scan, in: scans) else {
            delta = nil
            deltaBaseline = nil
            return
        }
        do {
            delta = try await client.getDiff(scanA: previous.id, scanB: scan.id)
            deltaBaseline = previous
            lastError = nil
        } catch {
            // A diff that cannot be produced leaves the tree unmarked; it
            // never blocks the surfaces that do not depend on it.
            delta = nil
            deltaBaseline = nil
        }
    }

    /// The newest complete scan of the same root that started BEFORE
    /// `scan` — the natural "last scan" baseline. Trailing slash cosmetic.
    public static func previousCompleteScan(before scan: Scan, in scans: [Scan]) -> Scan? {
        let trim = { (s: String) -> String in
            s.count > 1 && s.hasSuffix("/") ? String(s.dropLast()) : s
        }
        let root = trim(scan.rootPath)
        return scans
            .filter { $0.id != scan.id && $0.status == .complete && trim($0.rootPath) == root && $0.startedAt < scan.startedAt }
            .max { a, b in
                a.startedAt != b.startedAt ? a.startedAt < b.startedAt : a.id.uuidString < b.id.uuidString
            }
    }

    /// The other complete scans of the selected root, newest first — the
    /// compare picker's choices.
    public var comparableScans: [Scan] {
        guard let scan = selectedScan else { return [] }
        let trim = { (s: String) -> String in
            s.count > 1 && s.hasSuffix("/") ? String(s.dropLast()) : s
        }
        let root = trim(scan.rootPath)
        return scans
            .filter { $0.id != scan.id && $0.status == .complete && trim($0.rootPath) == root }
            .sorted { $0.startedAt > $1.startedAt }
    }

    /// Compare the selection against `compareWithID`, OLDER scan first so
    /// the deltas read forward in time whichever the user picked.
    public func loadComparison() async {
        guard let client, let scan = selectedScan, let otherID = compareWithID,
              let other = scans.first(where: { $0.id == otherID }) else {
            comparison = nil
            return
        }
        let (a, b) = other.startedAt <= scan.startedAt ? (other, scan) : (scan, other)
        do {
            comparison = try await client.getDiff(scanA: a.id, scanB: b.id)
            lastError = nil
        } catch {
            comparison = nil
            lastError = error.localizedDescription
    ...
    /// The mark for a tree row or treemap tile: only exact paths in the
    /// delta's `grown` list, never the scan root itself (it grows whenever
    /// anything under it does, which would outline the whole map).
    public func deltaMark(for path: String) -> DeltaMark? {
        guard let delta, path != delta.rootPath, path != selectedScan?.rootPath else { return nil }
        return Self.deltaMark(for: path, in: delta)
    }

    public static func deltaMark(for path: String, in diff: ScanDiff) -> DeltaMark? {
        guard let entry = diff.grown.first(where: { $0.path == path }), entry.delta > 0 else { return nil }
        let bytes = UInt64(entry.delta)
        return entry.before == nil ? .new(bytes: bytes) : .grown(bytes: bytes)
    }

    /// Evaluate `scan` (a completed scan of some root) against the previous
    /// complete scan of that root, and deliver at most ONE notification per
    /// scan id, ever. Returns the alert it delivered, or nil (disabled, no
    /// previous scan, already notified, nothing worth saying, or a fetch
    /// failed — a notification is never worth an error banner).
    @discardableResult
    public func notifyGrowthIfWarranted(for scan: Scan) async -> GrowthAlert? {
        guard growthAlertsEnabled, let client, let deliverer = growthAlertDeliverer,
              scan.status == .complete, !notifiedScans.contains(scan.id),
              let previous = Self.previousCompleteScan(before: scan, in: scans)
        else { return nil }
        // Both fetches or nothing: a half-informed notification ("grew 20 GB"
        // with no culprit and no reclaim line) is worse than waiting for the
        // next poll, and an unreachable server is exactly when the next poll
        // will do better. Not marked, so that retry may still speak once.
        let growth: Growth
        let hotspots: HotspotsSummary
        do {
            growth = try await client.getGrowth(root: scan.rootPath, groupBy: GrowthGroupBy.category.rawValue)
            hotspots = try await client.getHotspots(scanID: scan.id)
        } catch {
            return nil
        }
        guard let alert = GrowthAlert.evaluate(
            previous: previous, current: scan, growth: growth, hotspots: hotspots,
            thresholds: growthAlertThresholds
        ) else { return nil }
        // Mark BEFORE delivering: a delivery that throws or hangs must not
        // earn a second try on the next poll.
        notifiedScans.insert(scan.id)
        await deliverer.deliver(title: alert.title(size: Self.sizeText), body: alert.body(size: Self.sizeText), scanID: scan.id)
        return alert
    }

    /// Decimal sizes like the rest of the app (1 GB = 1000 MB — Finder's
    /// convention, and DesignKit's). PhantomCore does not depend on DesignKit,
    /// so the rule lives here too; pinned against Format.size by a test.
    nonisolated public static func sizeText(_ bytes: UInt64) -> String {
    ...
    /// The sidebar sparkline's data, computed from the scan list already in
    /// hand (no request): the COMPLETE scans of `root`, oldest first, as
    /// their headline disk sizes. Fewer than two points draw nothing —
    /// one dot is not a trend.
    public static func sparklineValues(root: String, in scans: [Scan]) -> [UInt64] {
        let trim = { (s: String) -> String in
            s.count > 1 && s.hasSuffix("/") ? String(s.dropLast()) : s
        }
        let target = trim(root)
        let points = scans
            .filter { $0.status == .complete && trim($0.rootPath) == target }
            .sorted { a, b in
                a.startedAt != b.startedAt ? a.startedAt < b.startedAt : a.id.uuidString < b.id.uuidString
            }
            .map(\.totalDiskSize)
