...
public protocol APIClientProtocol: Sendable {
    ...
    /// DELETE /scans/{id}: 204 on success; 409 while the scan is still
    /// running (cancel first); 404 when it is already gone.
    ...
    // Reclaim plans (1.1.0). POST answers 201 with the plan; the script is
    // text/plain, the ONE non-JSON success body on the API.
    func createPlan(scanID: UUID, maxTier: String) async throws -> ReclaimPlan
    func planScript(planID: UUID) async throws -> String
    /// GET /scans/series (1.1.0): the root's completed scans as a series
    /// plus the linear forecast. 404 when the root has no completed scan.
    func getGrowth(root: String, groupBy: String) async throws -> Growth
    /// GET /scans/{a}/diff/{b}: deltas read B − A. 400 for different roots,
    /// 409 while either scan is still running.
    func getDiff(scanA: UUID, scanB: UUID) async throws -> ScanDiff
}
...
public struct APIClient: APIClientProtocol {
    ...
    /// What the API log records as the sender of every request this client
    /// makes (phantom-cnr.7): `Phantom/<CFBundleShortVersionString>` from
    /// the app bundle, `Phantom/dev` when there is no bundle version (tests).
    /// The CLI and MCP server send `phantom-cli/<v>` and `phantom-mcp/<v>`.
    public static let userAgent: String = {
        let version = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String
        return "Phantom/\(version ?? "dev")"
    }()
    ...
    public static let defaultBaseURL = URL(string: "http://127.0.0.1:18770")!
    ...
    /// scheme (e.g. "localhost:18770") is not a usable HTTP base, so it too
    ...
    public func deleteScan(id: UUID) async throws {
        // 204 carries no body, so this bypasses the decoding `request` path.
        _ = try await perform("DELETE", "/scans/\(id.uuidString.lowercased())", query: [], bodyData: nil)
    }
    ...
    public func createPlan(scanID: UUID, maxTier: String) async throws -> ReclaimPlan {
        struct PlanRequest: Encodable { let maxTier: String }
        return try await request(
            "POST", "/scans/\(scanID.uuidString.lowercased())/plan", body: PlanRequest(maxTier: maxTier)
        )
    }

    /// The plan as a shell script: raw text, not decoded as JSON.
    public func getDiff(scanA: UUID, scanB: UUID) async throws -> ScanDiff {
        try await request("GET", "/scans/\(scanA.uuidString.lowercased())/diff/\(scanB.uuidString.lowercased())")
    }

    public func getGrowth(root: String, groupBy: String) async throws -> Growth {
        try await request(
            "GET", "/scans/series",
            query: [URLQueryItem(name: "root", value: root), URLQueryItem(name: "groupBy", value: groupBy)]
        )
    }

    public func planScript(planID: UUID) async throws -> String {
        let (data, _) = try await perform(
            "GET", "/plans/\(planID.uuidString.lowercased())/script", query: [], bodyData: nil
        )
        guard let text = String(data: data, encoding: .utf8) else {
            throw APIError.decodingFailed("plan script is not UTF-8")
        }
        return text
    }
