...
final class ScansModelTests: XCTestCase {
    ...
    // MARK: remove (DELETE /scans/{id})

    func testRemoveSelectedScanDropsRowAndSelectsTheRowBelow() async throws {
        let mock = MockAPIClient()
        let a = makeScan(rootPath: "/a", status: .complete)
        let b = makeScan(rootPath: "/b", status: .complete)
        let c = makeScan(rootPath: "/c", status: .complete)
        mock.scans = [a, b, c]
        let model = ScansModel(client: mock)
        await model.refresh()
        await model.select(scanID: b.id)

        await model.remove(id: b.id)
        XCTAssertEqual(model.scans.map(\.id), [a.id, c.id])
        XCTAssertEqual(mock.deletedScanIDs, [b.id], "exactly one DELETE, for the removed scan")
        XCTAssertEqual(model.selectedScanID, c.id, "selection moves to the row below")
        XCTAssertNil(model.lastError)
    }

    func testRemoveLastRowSelectsTheRowAbove() async {
        let mock = MockAPIClient()
        let a = makeScan(rootPath: "/a", status: .complete)
        let b = makeScan(rootPath: "/b", status: .complete)
        mock.scans = [a, b]
        ...
        await model.remove(id: b.id)
        XCTAssertEqual(model.scans.map(\.id), [a.id])
        XCTAssertEqual(model.selectedScanID, a.id)
    }

    func testRemoveOnlyRowClearsSelection() async {
        let mock = MockAPIClient()
        let only = makeScan(status: .complete)
        mock.scans = [only]
        let model = ScansModel(client: mock)
        await model.refresh()
        await model.select(scanID: only.id)

        await model.remove(id: only.id)
        XCTAssertEqual(model.scans, [])
        XCTAssertNil(model.selectedScanID)
        XCTAssertNil(model.selectedScan)
    }

    func testRemoveUnselectedScanLeavesSelectionAlone() async {
        let mock = MockAPIClient()
        let a = makeScan(rootPath: "/a", status: .complete)
        let b = makeScan(rootPath: "/b", status: .cancelled)
        mock.scans = [a, b]
        let model = ScansModel(client: mock)
        await model.refresh()
        await model.select(scanID: a.id)

        await model.remove(id: b.id)
        XCTAssertEqual(model.scans.map(\.id), [a.id])
        XCTAssertEqual(model.selectedScanID, a.id, "removing another row must not disturb the selection")
    }

    func testRemoveRunningScanIsRefusedAndKeepsTheRow() async throws {
        let mock = MockAPIClient()
        let running = makeScan()
        mock.scans = [running]
        let model = ScansModel(client: mock)
        ...
        await model.remove(id: running.id)
        let message = try XCTUnwrap(model.lastError)
        XCTAssertTrue(message.contains("still running"), "got: \(message)")
        XCTAssertEqual(model.scans.map(\.id), [running.id], "409 must not drop the row")
        XCTAssertEqual(mock.scans.map(\.id), [running.id], "and the mock still holds it")
    }

    func testRemoveFailureSurfacesErrorAndKeepsTheRow() async throws {
        let mock = MockAPIClient()
        let complete = makeScan(status: .complete)
        mock.scans = [complete]
        let model = ScansModel(client: mock)
        await model.refresh()
        await model.select(scanID: complete.id)

        mock.failWith = .serverUnreachable("down")
        await model.remove(id: complete.id)
        XCTAssertNotNil(model.lastError)
        XCTAssertEqual(model.scans.map(\.id), [complete.id])
        XCTAssertEqual(model.selectedScanID, complete.id, "a failed removal changes nothing")
    }

    func testNeighbourIDPrefersBelowThenAboveThenNil() {
        let a = makeScan(rootPath: "/a", status: .complete)
        let b = makeScan(rootPath: "/b", status: .complete)
        let c = makeScan(rootPath: "/c", status: .complete)
        XCTAssertEqual(ScansModel.neighbourID(of: a.id, in: [a, b, c]), b.id)
        XCTAssertEqual(ScansModel.neighbourID(of: b.id, in: [a, b, c]), c.id)
        XCTAssertEqual(ScansModel.neighbourID(of: c.id, in: [a, b, c]), b.id)
        XCTAssertNil(ScansModel.neighbourID(of: a.id, in: [a]))
        XCTAssertNil(ScansModel.neighbourID(of: UUID(), in: [a, b]), "unknown id has no neighbour")
    }
