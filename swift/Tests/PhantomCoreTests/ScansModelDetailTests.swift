...
final class ScansModelDetailTests: XCTestCase {
    ...
    // MARK: reclaim script (v1.1 "Copy as script")

    private func makePlan(scanID: UUID) throws -> ReclaimPlan {
        try Wire.decoder().decode(
            ReclaimPlan.self,
            from: Data(#"""
            {"planId":"7d0f3a9c-2b41-4c8e-9f65-0a1b2c3d4e5f","scanId":"\#(scanID.uuidString.lowercased())",
             "rootPath":"/r","createdAt":"2026-03-17T15:00:00.000000Z","maxTier":"safe","minBytes":0,
             "items":[],"itemCount":0,"expectedFreedBytes":0,"skipped":{"review":0,"aboveTier":0,"belowMinBytes":0,"tracked":0}}
            """#.utf8))
    }

    func testReclaimScriptBuildsASafePlanForTheSelectedScanAndFetchesItsScript() async throws {
        let mock = MockAPIClient()
        let scan = makeScan(status: .complete)
        mock.scans = [scan]
        mock.planToReturn = try makePlan(scanID: scan.id)
        mock.scriptToReturn = "#!/bin/sh\napply '/r/target'\n"
        let model = ScansModel(client: mock)
        await model.refresh()
        await model.select(scanID: scan.id)

        let script = await model.reclaimScript()
        XCTAssertEqual(script, "#!/bin/sh\napply '/r/target'\n")
        XCTAssertEqual(mock.planRequests.map(\.scanID), [scan.id])
        XCTAssertEqual(mock.planRequests.map(\.maxTier), ["safe"], "the app asks for SAFE groups only")
        XCTAssertEqual(mock.scriptRequests, [try makePlan(scanID: scan.id).planId])
        XCTAssertNil(model.lastError)
    }

    func testReclaimScriptSurfacesFailureAndReturnsNil() async {
        let mock = MockAPIClient()
        let scan = makeScan(status: .complete)
        mock.scans = [scan]
        let model = ScansModel(client: mock)
        await model.refresh()
        await model.select(scanID: scan.id)
        // planToReturn nil → the mock's 409.
        let script = await model.reclaimScript()
        XCTAssertNil(script)
        XCTAssertNotNil(model.lastError)
        XCTAssertTrue(mock.scriptRequests.isEmpty, "no plan, no script fetch")
    }

    func testReclaimScriptWithoutASelectionDoesNothing() async {
        let mock = MockAPIClient()
        let model = ScansModel(client: mock)
        let script = await model.reclaimScript()
        XCTAssertNil(script)
        XCTAssertTrue(mock.planRequests.isEmpty)
    }
