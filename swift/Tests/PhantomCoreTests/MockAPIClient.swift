...
final class MockAPIClient: APIClientProtocol, @unchecked Sendable {
    ...
    private(set) var deletedScanIDs: [UUID] = []

    /// Mirrors the API: 404 when unknown, 409 while still running (cancel
    /// first), otherwise the row is gone and the call returns nothing.
    func deleteScan(id: UUID) async throws {
        ...
        guard scans[idx].isTerminal else {
            throw APIError.httpError(
                status: 409, message: "scan \(id) is still running; cancel it before deleting"
            )
        }
        scans.remove(at: idx)
    ...
    /// The plan the mock hands back (nil → a 409 "still running", the API's
    /// answer for a scan without hotspots yet); the script text likewise.
    var planToReturn: ReclaimPlan?
    var scriptToReturn: String = "#!/bin/sh\necho would move\n"
    private(set) var planRequests: [(scanID: UUID, maxTier: String)] = []
    private(set) var scriptRequests: [UUID] = []

    func createPlan(scanID: UUID, maxTier: String) async throws -> ReclaimPlan {
        try checkFailure()
        planRequests.append((scanID, maxTier))
        guard let plan = planToReturn else {
            throw APIError.httpError(
                status: 409, message: "scan \(scanID) is still running; results are available once it finishes"
            )
        }
        return plan
    }
    ...
    func planScript(planID: UUID) async throws -> String {
        try checkFailure()
        scriptRequests.append(planID)
        return scriptToReturn
    }

    /// Diff (Phase 4 delta/compare): keyed by the (a, b) pair so a test can
    /// script the previous-scan delta and an explicit comparison apart.
    var diffsByPair: [String: ScanDiff] = [:]
    private(set) var diffRequests: [(a: UUID, b: UUID)] = []
    func getDiff(scanA: UUID, scanB: UUID) async throws -> ScanDiff {
        try checkFailure()
        diffRequests.append((scanA, scanB))
        guard let diff = diffsByPair["\(scanA.uuidString.lowercased())|\(scanB.uuidString.lowercased())"] else {
            throw APIError.httpError(status: 409, message: "scan \(scanA) is running — only complete scans have results to diff")
        }
        return diff
    }

    /// Growth (1.1.0 Phase 4): the series to hand back, keyed by nothing —
    /// one root per test. Nil answers the API's 404 (no completed scans).
    var growthToReturn: Growth?
    private(set) var growthRequests: [(root: String, groupBy: String)] = []
    func getGrowth(root: String, groupBy: String) async throws -> Growth {
        try checkFailure()
        growthRequests.append((root, groupBy))
        guard let growth = growthToReturn else {
            throw APIError.httpError(status: 404, message: "not found: no completed scans of \(root)")
        }
        return growth
    }
