...
At scan completion, before the entry filter drops sub-1-MiB rows, every
directory's `diskSize`/`logicalSize` is replaced by the sum over ALL
descendant files at full depth (walking each file's ancestor chain up to the
scan root; the first ancestor outside the scan stops the walk). The filter
then keeps files ≥ 1 MiB and — since 1.1.1 — directories whose aggregate is
≥ 1 MiB (plus the scan root, hotspot `topPaths` and mount points with their
ancestor chains, regardless of size). Consequence: a persisted directory
row's aggregate can exceed the sum of its persisted children — the
difference is the filtered remainder: small files AND small subtrees. A
kept directory may have no persisted children at all. The treemap below
leans on this.
...
| `topPaths` within a group | deduped size descending, then path ascending; every root with ≥ 1 GiB private, at least 5, at most 25 (1.1.1 — was a flat cap of 5) |
...
at ≥ 1 GiB (inclusive); top paths per group: every root with ≥ 1 GiB
private, floor 5, cap 25 (1.1.1); git activity files
