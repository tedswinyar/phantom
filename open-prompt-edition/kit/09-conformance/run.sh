...
# listening anywhere in this range. init.sh rewrites 18770 to the stamped
# project's real prod port, so this stays correct after stamping. (Until
# 2026-09-09 the max was a stray 18309 — a 9.5k-port refusal, not a ladder.)
PROD_PORT=18770
PROD_PORT_MAX=18779
...
cleanup() {
  ...
  [ -n "${VOLUME_FILE:-}" ] && rm -f "$VOLUME_FILE"
  [ -n "${GROWTH_FILE:-}" ] && rm -f "$GROWTH_FILE"
}
trap cleanup EXIT
...
# An APFS pure clone of two.bin (kit 1.1.0): st_blocks reports a full
# megabyte on it, yet the clone group is charged once and deleting it frees
# nothing — the privateSize checks below. Named to sort after two.bin so
# two.bin keeps the persisted row.
cp -c "$SCAN_DIR/two.bin" "$SCAN_DIR/zz-clone-of-two.bin"
...
t "create returns all fifteen wire keys" \
  bash -c "printf '%s' '$CREATED' | jq -e 'has(\"id\") and has(\"rootPath\") and has(\"status\") and has(\"startedAt\") and has(\"finishedAt\") and has(\"totalDiskSize\") and has(\"totalLogicalSize\") and has(\"fileCount\") and has(\"dirCount\") and has(\"errorCount\") and has(\"unreadablePaths\") and has(\"totalPrivateSize\") and has(\"totalSharedSize\") and has(\"failureReason\") and has(\"progress\")'"
t "crossVolumes is an accepted request field (one filesystem by default)" \
  bash -c "[ \$(curl -s -o /dev/null -w '%{http_code}' -X POST -H 'x-api-key: $KEY' -H 'content-type: application/json' -d '{\"rootPath\":\"$SCAN_DIR\",\"crossVolumes\":false}' '$BASE/scans') = 202 ]"
...
t "root dir entry carries full-walk descendant counts" \
  bash -c "curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID/entry?path=$SCAN_DIR' | jq -e '.fileCount == 6 and .dirCount == 1'"
...
t "totalDiskSize counts a hardlinked inode once AND a clone group once" \
  bash -c "curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID' | jq -e '.totalDiskSize < 4194304 and .totalDiskSize >= 3145728'"
t "the duplicate link and the clone have no persisted row of their own" \
  bash -c "curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID/files' | jq -e '[.[].name] | index(\"zz-linked.bin\") == null and index(\"zz-clone-of-two.bin\") == null and index(\"one.bin\") != null and index(\"two.bin\") != null'"

# ---------------------------------------------------------------------------
# Three sizes (kit 1.1.0, 06-interchange "Three sizes"): the clone pair
# shares a cloneId; the persisted member reports privateSize 0 (its twin
# pins every block) and sharedSize == diskSize; the root's split covers the
# whole tree (every sharing group is wholly inside it), so its privateSize
# equals the scan's totalDiskSize and totalPrivateSize.
# ---------------------------------------------------------------------------
t "a pure clone carries cloneId, privateSize 0, sharedSize == diskSize, both clone flags" \
  bash -c "curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID/entry?path=$SCAN_DIR/two.bin' | jq -e '.cloneId != null and .privateSize == 0 and .sharedSize == .diskSize and (.flags | index(\"sharesAllBlocks\") != null) and (.flags | index(\"mayShareBlocks\") != null)'"
t "an ordinary file is fully private with no flags" \
  bash -c "curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID/entry?path=$SCAN_DIR/node_modules/chunk.bin' | jq -e '.cloneId == null and .privateSize == .diskSize and .sharedSize == 0 and .flags == []'"
t "the root's privateSize equals totalDiskSize (every group wholly inside) and totalPrivateSize" \
  bash -c "diff <(curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID/entry?path=$SCAN_DIR' | jq -c '[.privateSize, .sharedSize]') <(curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID' | jq -c '[.totalDiskSize, 0]') && diff <(curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID/entry?path=$SCAN_DIR' | jq -c '[.privateSize, .sharedSize]') <(curl -s -H 'x-api-key: $KEY' '$BASE/scans/$ID' | jq -c '[.totalPrivateSize, .totalSharedSize]')"
...
t "reclaim estimate equals the group's privateSize (plain files: == diskSize)" \
  bash -c "printf '%s' '$HOTSPOTS' | jq -e '.reclaimEstimate == .groups[0].privateSize and .groups[0].privateSize == .groups[0].diskSize and .groups[0].diskSize >= 1048576'"
t "group carries riskTier/why/rebuildCost; node_modules without a lockfile is caution" \
  bash -c "printf '%s' '$HOTSPOTS' | jq -e '.groups[0] | .riskTier == \"caution\" and (.why | endswith(\".\")) and .rebuildCost.kind == \"download\" and has(\"toolEstimate\") and .toolEstimate == null'"
t "rebuildCost has exactly the fixture's key set" \
  bash -c "diff <(printf '%s' '$HOTSPOTS' | jq -S '.groups[0].rebuildCost | keys') <(jq -S '.groups[0].rebuildCost | keys' '$FIXTURES/hotspots-summary.json')"
...
# ---------------------------------------------------------------------------
# Volume (kit 1.1.0, 06-interchange "VolumeStatus"): an OBJECT whose key set
# and `hidden` key set match the fixture; the container arithmetic holds;
# with a completed scanId the split is relative to that scan. Nothing runs
# (snapshots are not asked for).
# ---------------------------------------------------------------------------
# The body goes through a FILE: its `note` carries apostrophes, which the
# single-quoted interpolation every other check uses cannot hold.
VOLUME_FILE="$(mktemp /tmp/phantom-conformance-volume.XXXXXX)"
api GET "/volume?path=$SCAN_DIR&scanId=$ID" > "$VOLUME_FILE"
t "volume has exactly the fixture's key set" \
  bash -c "diff <(jq -S 'keys' '$VOLUME_FILE') <(jq -S 'keys' '$FIXTURES/volume-status.json')"
t "volume.hidden has exactly the fixture's key set" \
  bash -c "diff <(jq -S '.hidden | keys' '$VOLUME_FILE') <(jq -S '.hidden | keys' '$FIXTURES/volume-status.json')"
t "usedBytes is total − free and volumeUsedBytes ≤ usedBytes (null allowed off APFS)" \
  jq -e '.usedBytes == .totalBytes - .freeBytes and (.volumeUsedBytes == null or .volumeUsedBytes <= .usedBytes)' "$VOLUME_FILE"
t "purgeableBytes is importantUsage − available floored at 0 (both null together)" \
  jq -e 'if .importantUsageBytes == null then .purgeableBytes == null else .purgeableBytes == ([.importantUsageBytes - .availableBytes, 0] | max) end' "$VOLUME_FILE"
t "hidden split is relative to the named scan (scannedBytes == its totalDiskSize)" \
  jq -e --arg id "$ID" --argjson t "$(printf '%s' "$TERMINAL" | jq .totalDiskSize)" '.hidden.scanId == $id and .hidden.scannedBytes == $t and .hidden.snapshotSuggestion == null' "$VOLUME_FILE"
t "snapshots not asked: count and list are null" \
  jq -e '.snapshotCount == null and .snapshots == null' "$VOLUME_FILE"

# ---------------------------------------------------------------------------
# Growth (kit 1.1.0, 06-interchange "Growth"): the scanned tree has two
# completed scans (the diff section rescanned it unchanged), so the series
# has two points, a flat total line and a forecast with bytesPerDay 0 and
# no full date. Key sets vs the fixture at every depth.
# ---------------------------------------------------------------------------
GROWTH_FILE="$(mktemp /tmp/phantom-conformance-growth.XXXXXX)"
curl -s -G -H "x-api-key: $KEY" --data-urlencode "root=$SCAN_DIR" "$BASE/scans/series" > "$GROWTH_FILE"
t "growth has exactly the fixture's key set" \
  bash -c "diff <(jq -S 'keys' '$GROWTH_FILE') <(jq -S 'keys' '$FIXTURES/growth.json')"
t "growth point has exactly the fixture point's key set" \
  bash -c "diff <(jq -S '.points[0] | keys' '$GROWTH_FILE') <(jq -S '.points[0] | keys' '$FIXTURES/growth.json')"
t "growth forecast has exactly the fixture forecast's key set" \
  bash -c "diff <(jq -S '.forecast | keys' '$GROWTH_FILE') <(jq -S '.forecast | keys' '$FIXTURES/growth.json')"
t "growth points are the root's completed scans, oldest first" \
  jq -e --arg id "$ID" '(.points | length) >= 2 and .points[0].scanId == $id and ([.points[].startedAt] == ([.points[].startedAt] | sort))' "$GROWTH_FILE"
t "total series is one line equal to the points" \
  jq -e '.groupBy == "total" and (.series | length) == 1 and .series[0].key == "total" and .series[0].values == [.points[].totalDiskSize]' "$GROWTH_FILE"
t "an unchanged tree forecasts bytesPerDay 0 and no full date" \
  jq -e '.forecast.method == "linear" and .forecast.bytesPerDay == 0 and .forecast.daysUntilFull == null and .forecast.projectedFullAt == null and (.forecast.caveat | length) > 0' "$GROWTH_FILE"
t "series without root is 400 with error shape" \
  bash -c "curl -s -H 'x-api-key: $KEY' '$BASE/scans/series' | jq -e '.error | test(\"root\")'"
t "series of an unscanned root is 404" \
  bash -c "[ \$(curl -s -o /dev/null -w '%{http_code}' -H 'x-api-key: $KEY' '$BASE/scans/series?root=/no/such/root') = 404 ]"
