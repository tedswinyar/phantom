...
3. Terminal handoff, in this order: fold totals into the scan row → compute
   ...
   persistence filter (directories carry full-depth size aggregates AND
   full-depth descendant `fileCount`/`dirCount` — sub-1-MiB files count in
   all of them — and a directory ROW survives when that aggregate is ≥ 1 MiB
   or the directory is the root, a hotspot `topPath` or a mount point, with
   its ancestors; files ≥ 1 MiB inclusive survive as rows with null counts;
   the summary is an INPUT to the filter, so classify first)
...
4. After every SUCCESSFUL terminal persist, prune the collection to the
   newest 25 scans of each root (100 overall), then to a 2 GiB byte budget
   that evicts the globally oldest scan of any root still holding more than
   two — never a root's last two while its newest scan is 30 days old or
   younger; a root idle longer than that has no floor
   (`../02-contracts/api-protocol.md`, GET /scans). A prune failure must
   not fail the scan — it is already
   persisted; log and move on.
