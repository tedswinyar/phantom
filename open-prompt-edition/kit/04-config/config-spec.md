...
| `PHANTOM_API_URL` | Base URL clients call | `http://127.0.0.1:18770` (else the published `api_url` file — see api-protocol) |
...
| `PHANTOM_SUPERVISOR_PID` | (API only, 1.1.0) pid of the process that owns this server; the API polls it every second and shuts down cleanly (cancelling in-flight scans) once it is gone. The app sets it to its own pid. Unset = live until signalled | — |
...
| `dev` | `<data_dir>/phantom/phantom-dev.db` | 18780 | same as prod |
...
configured port. The server binds loopback only. (Context: 18770 is Phantom's
registered port in its local port registry, in the 187xx range its sibling
tools use; 1.0 shipped on 8768, which a third-party agent squatted on
2026-09-09. The ladder 18770–18779 crosses neighboring registered ports,
tolerated ONLY because every supervisor parses the announcement and clients
read the published `api_url` file.)
