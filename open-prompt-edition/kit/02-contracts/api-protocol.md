...
- `200` → bare JSON array of scan wire views: persisted scans (with
  ...
  **100 overall** (entries and totals cascade; both figures are product
  decisions, overridable by the operator through `PHANTOM_KEEP_SCANS_PER_ROOT`
  / `PHANTOM_KEEP_SCANS`). Files within a scan are the thing that paginates.
  Consequence for clients: a scan id is NOT durable forever — a root's 26th
  completion evicts that root's oldest scan, and later reads of it are clean
  404s; scans of other roots are never evicted by it (1.1.0; 1.0 pruned to
  the newest 25 regardless of root).
...
- Shares the results gate (409 while running, 404 unknown scan); `400`
  without `path`; `404` for a path the scan did not record (a file under
  1 MiB has no row — explain its directory).
...
- `200` → a `Growth` OBJECT: the completed scans of `root` (matched like
  ...
  up to `totalDiskSize` as `other`; `extension` = the per-extension totals,
