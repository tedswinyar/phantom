...
persists is deliberately bounded: every file of 1 MiB or more, and every
directory whose subtree is 1 MiB or more (hotspot roots and mount points
always), gets its own row; smaller files and smaller subtrees fold into the
nearest persisted ancestor's totals. The per-type breakdown and the Reclaimable classification are
