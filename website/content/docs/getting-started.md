...
- **The treemap** shows where the bytes physically are.
...
Phantom's UI uses plain names by default — Scan, Reclaimable, treemap. The
spooky vocabulary the app shipped with in 1.0 (Haunt, Specter Map, Restless
Spirits, Crypt) is still there as an opt-in: Settings → General → "Use
spooky names". The site's copy follows the default so it matches what you
see on first launch.
