...
**What gets stored?** Not the raw walk. A scan persists every file of
1&nbsp;MiB or more, and every directory whose subtree is 1&nbsp;MiB or more
(plus hotspot roots and mount points regardless of size), as queryable rows;
smaller files and smaller directory subtrees fold into the nearest persisted
ancestor's aggregated totals. The per-type totals and the Reclaimable
...
when its default (18770) is taken and prints the bound address on stdout;
