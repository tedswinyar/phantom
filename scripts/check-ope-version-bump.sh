...
#
# ONE RELEASE, ONE BUMP: an UNRELEASED version absorbs further contract
# changes. If VERSION at the head is already strictly above the VERSION at
# the last release tag (`v*`) reachable from the base, the contract is still
# being assembled for a release nobody has, and more changes may land under
# the same number (v1.1 Phases 1–6 all ship as 1.1.0 — VERSIONING.md keeps
# the app and OPE numbers aligned, so bumping per phase would desync them).
# Once that version is tagged, the next contract change needs a new bump.
# No release tag at all ⇒ the strict per-push rule applies.
#
...
# Is $1 (the head's VERSION) strictly above the VERSION at the last release
# tag reachable from commit $2? Prints the released version on success.
# Fails when there is no tag, no VERSION at the tag, or no strict increase.
unreleased_above_tag() {
    local version="$1" from="$2" tag released highest
    tag=$(git describe --tags --abbrev=0 --match 'v*' "$from" 2>/dev/null) || return 1
    released=$(git show "$tag:$VERSION_FILE" 2>/dev/null | tr -d '[:space:]')
    [ -n "$released" ] || return 1
    highest=$(printf '%s\n%s\n' "$released" "$version" | sort -V | tail -1)
    [ "$version" != "$released" ] && [ "$highest" = "$version" ] || return 1
    printf '%s' "$released"
}
...
if [ "$WORKTREE" = 1 ]; then
    ...
    if [ -n "$old_version" ]; then
        ...
        if [ "$new_version" = "$old_version" ]; then
            if released=$(unreleased_above_tag "$new_version" HEAD); then
                ok_msg "ope-version: contract changes land in UNRELEASED v${new_version} (last release v${released})"
                exit 0
            fi
        fi
...
if [ -n "$old_version" ]; then
    highest=$(printf '%s\n%s\n' "$old_version" "$new_version" | sort -V | tail -1)
    if [ "$new_version" = "$old_version" ]; then
        if released=$(unreleased_above_tag "$new_version" "$BASE"); then
            ok_msg "ope-version: contract files changed under UNRELEASED v${new_version} (last release v${released}); one release, one bump"
            exit 0
        fi
    fi
    if [ "$new_version" = "$old_version" ] || [ "$highest" != "$new_version" ]; then
        ...
        fail_msg "unless v${new_version} is still unreleased (above the last v* tag), which it is not."
        fail_msg ""
