...
# A git hook exports GIT_DIR (and friends) to everything it runs. From the main
# checkout that is the relative ".git", which the suites' temp repos happen to
# resolve harmlessly; from a WORKTREE it is an absolute path to
# .git/worktrees/<name>, so every `git init`/`git push` a test makes in its own
# mktemp repo silently talks to the worktree's gitdir instead — test-relay-hook
# lost 13 of 16 and test-hook-markers 6 of 22 on the first branch push from an
# agent worktree (2026-09-16), while the same tests passed when run by hand.
# Scrub the hook's git environment here so verify sees the same world however
# it was started.
...
run_scripts() {
  ...
  # A glob that matches NOTHING must be a FAILURE, not a pass (template defect
  # spooky-shell-rlm): scripts/tests/ exists — run_suite gates on the directory
  ...
  if [ "$matched" -eq 0 ]; then
    echo "run_scripts: scripts/tests/ exists but contains no test-*.sh — a gate that runs nothing must not report PASS (spooky-shell-rlm)" >&2
    rc=1
  fi
  # Would HEAD survive the public cut? Same exclusion list and sweep as
  # recut-public.sh (scripts/lib/public-sweep.sh), run on every push so an
  # identifier is caught the day it is committed, not on release day after the
  # release is live (phantom-mj7, 1.1.0). The script is withheld from the public
  # cut — it carries the terms — so its absence there is not a failure.
  if [ -x scripts/public-sweep.sh ]; then
    echo "── scripts suite: public identifier sweep (HEAD)"
    scripts/public-sweep.sh HEAD || rc=1
  fi
  # Every source of "what version is this?" agrees (VERSIONING.md, phantom-cnr.6):
  # Version.swift, the Cargo workspace, OPE VERSION, the plugin manifests, the
  # CHANGELOG heading. rust/Cargo.toml sat at 1.0.0 for two weeks of 1.1 work
  # because only release.sh ever compared them.
  if [ -x scripts/check-version-alignment.sh ]; then
    echo "── scripts suite: version alignment"
    scripts/check-version-alignment.sh || rc=1
  fi
