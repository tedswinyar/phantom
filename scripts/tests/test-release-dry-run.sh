...
# The DMG stub leaves what release.sh inspects afterwards: the bundled CLI, which
# must report exactly the release version. It echoes the PHANTOM_GIT_DESCRIBE
# release.sh exports for the build (phantom-cnr.6) — so this also pins that the
# export happens; an unset variable would print "phantom " and fail the assert.
cat > "$R/scripts/build-dmg.sh" <<'EOF'
#!/bin/sh
mkdir -p build/Phantom.app/Contents/Helpers
printf '#!/bin/sh\necho "phantom %s"\n' "${PHANTOM_GIT_DESCRIBE#v}" > build/Phantom.app/Contents/Helpers/phantom
chmod +x build/Phantom.app/Contents/Helpers/phantom
exit 0
EOF
chmod +x "$R/scripts/build-dmg.sh"
...
# --- The bundled CLI must report exactly the release version (phantom-cnr.6) --
# A build that ignored PHANTOM_GIT_DESCRIBE (or a dirty/untagged tree) prints a
# describe suffix or another version; release.sh must refuse before notes/tag.
cat > "$R/scripts/build-dmg.sh" <<'EOF'
#!/bin/sh
mkdir -p build/Phantom.app/Contents/Helpers
printf '#!/bin/sh\necho "phantom 9.9.9 (v9.9.8-3-gabc1234-dirty)"\n' > build/Phantom.app/Contents/Helpers/phantom
chmod +x build/Phantom.app/Contents/Helpers/phantom
exit 0
EOF
printf '# Changelog\n' > "$R/CHANGELOG.md"   # the first dry run above wrote notes; start clean
OUT="$(cd "$R" && env -u SKIP_NOTARIZE PATH="$SHIM_PATH" DRY_RUN=1 ./scripts/release.sh 9.9.9 2>&1)"
STATUS=$?
t "a CLI labelled as an untagged build blocks the release" [ "$STATUS" -ne 0 ]
t "the block names what the CLI said and what the release is" \
  bash -c "echo \"\$1\" | grep -q \"reports 'phantom 9.9.9 (v9.9.8-3-gabc1234-dirty)', the release is 9.9.9\"" _ "$OUT"
t "the block happens before notes are generated" \
  bash -c "! grep -q notes '$R/CHANGELOG.md'"
# Restore the honest stub for the runs below.
cat > "$R/scripts/build-dmg.sh" <<'EOF'
#!/bin/sh
mkdir -p build/Phantom.app/Contents/Helpers
printf '#!/bin/sh\necho "phantom %s"\n' "${PHANTOM_GIT_DESCRIBE#v}" > build/Phantom.app/Contents/Helpers/phantom
