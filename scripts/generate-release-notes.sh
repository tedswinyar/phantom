...
# Curated highlights (docs/releases/<version>.md) go under the matching
# version header — git-cliff regenerates the file, so they cannot live in it.
# shellcheck source=lib/release-notes.sh
. "$ROOT_DIR/scripts/lib/release-notes.sh"
ENRICHED="$(insert_highlights CHANGELOG.md "$ROOT_DIR/docs/releases")"
echo "generate-release-notes: $ENRICHED version(s) carry curated highlights"
