...
pub fn verify_plan(
    ...
    let items: Vec<VerifyItem> = plan
        ...
        .map(|item| {
            ...
            // None == no plan path had a row in `before`. Since 1.1.1 every
            // hotspot topPath keeps its row regardless of size (persist.rs),
            // and plan paths are always topPaths, so this branch is
            // unreachable for a plan built by this server; it stays for a
            // plan whose paths were edited by hand or whose scan rows were
            // lost (test_plans: plan_uses_persisted_private_bytes…).
