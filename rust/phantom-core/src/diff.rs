...
/// disk sizes; null means the directory has NO ROW in that scan — absent
/// (created or deleted between the two) OR below the 1 MiB directory
/// threshold there (1.1.1, phantom-cnr.10) — and its side counts as 0 in
/// `delta`, which is then right to within `DIFF_MIN_DELTA`.
