...
fn migrate_step(conn: &Connection, from: i64) -> Result<()> {
    match from {
        ...
        4 => {
            // v5 (phantom-mkn.1 / mkn.2, v1.1 Phase 1): what deletion
            // actually frees. entries.private_size / shared_size are bytes
            // (files: the kernel's APFS PRIVATESIZE, 0 for a hard link or a
            // pure clone; dir rows: the subtree rollup); clone_id is the
            // APFS clone-group id, NULL unless the file is a pure clone;
            // flags is the `scan::EntryFlags` bitmask (dataless, purgeable,
            // sparse, clone bits, firmlink, mount point). scans gains the
            // root's private/shared totals. NULL == not recorded: rows
            // persisted before v5 (the walk is gone; assuming
            // private == disk would restate the lie v1.0 told).
            conn.execute_batch(
        ...
        5 => {
            // v6 (v1.1 Phase 3, phantom-aoa): running scans are persisted at
            // start, so a server restart can mark them instead of losing
            // them. failure_reason carries the walker's error for a failed
            // scan, or the cold-start marker ("interrupted: …") for a scan
            // the server was running when it stopped. NULL for every other
            // status and for rows persisted before v6.
            conn.execute_batch("ALTER TABLE scans ADD COLUMN failure_reason TEXT;")?;
        }
        6 => {
            // v7 (v1.1 Phase 3, phantom-mkn.7): reclaim plans. A plan is the
            // dry-run an agent confirms before anything moves; it is stored
            // as its own wire JSON (like scans.hotspots) so `phantom verify`
            // can compare the promise with a rescan days later. scan_id is
            // NOT a foreign key on purpose: the plan must outlive its scan's
            // retention (verification then measures the root delta only).
            conn.execute_batch(
                "CREATE TABLE plans (
                    id         TEXT PRIMARY KEY,
...
mod tests {
    ...
    /// Build a database frozen at v4 (pre-clone-awareness).
    fn v4_database() -> Connection {
        let conn = v2_database();
        apply_migration(&conn, 2).unwrap();
        apply_migration(&conn, 3).unwrap();
        let v: i64 = conn
            .pragma_query_value(None, "user_version", |r| r.get(0))
            .unwrap();
        assert_eq!(v, 4, "precondition: database is at v4");
        conn
    }

    /// Additive-migration promise for the v5 arm: rows written under v4
    /// survive with every new column NULL (not recorded ≠ zero, and ≠
    /// "private == disk" — the assumption v1.0 made and v1.1 stopped making).
    #[test]
    fn v4_rows_survive_the_v5_migration_with_null_share_columns() {
        ...
        let row: (Option<i64>, Option<i64>, Option<i64>, Option<i64>) = conn
            .query_row(
                "SELECT private_size, shared_size, clone_id, flags
                 FROM entries WHERE path = '/haunt'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?)),
            )
            .unwrap();
        assert_eq!(row, (None, None, None, None), "pre-v5 entry rows carry NULLs");
        let totals: (Option<i64>, Option<i64>) = conn
            .query_row(
                "SELECT total_private_size, total_shared_size FROM scans WHERE id = 'scan-1'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .unwrap();
        assert_eq!(totals, (None, None), "pre-v5 scan rows carry NULL totals");
    }

    /// The v5 arm is transactional: with the LAST column pre-created, the
    /// final ALTER fails and every earlier ALTER must roll back with the
    /// version — neither a half-applied arm nor a version ahead of the
    /// schema.
    #[test]
    fn failed_v5_migration_rolls_back_every_alter() {
        let conn = v4_database();
        conn.execute_batch("ALTER TABLE scans ADD COLUMN total_shared_size INTEGER;")
            .unwrap();

        let err = apply_migration(&conn, 4);
        assert!(err.is_err(), "duplicate-column ALTER must fail");
        let v: i64 = conn
            .pragma_query_value(None, "user_version", |r| r.get(0))
            .unwrap();
        assert_eq!(v, 4, "version must stay at 4 when the v5 arm rolls back");
        assert!(
            conn.prepare("SELECT private_size FROM entries").is_err(),
            "the arm's first ALTER must roll back with the last"
        );
    }

    #[test]
    fn v6_adds_failure_reason_and_leaves_old_rows_null() {
        let conn = v4_database();
        conn.execute_batch(
            "INSERT INTO scans (id, root_path, status, started_at, finished_at,
                total_disk_size, total_logical_size, file_count, dir_count,
                error_count, hotspots, unreadable_paths)
             VALUES ('scan-1', '/r', 'failed', '2026-03-17T14:30:00.000000Z',
                '2026-03-17T14:31:00.000000Z', 0, 0, 0, 0, 0, NULL, NULL);",
        )
        .unwrap();
        apply_migration(&conn, 4).unwrap();
        apply_migration(&conn, 5).unwrap();
        let v: i64 = conn
            .pragma_query_value(None, "user_version", |r| r.get(0))
            .unwrap();
        assert_eq!(v, 6);
        let reason: Option<String> = conn
            .query_row("SELECT failure_reason FROM scans WHERE id = 'scan-1'", [], |r| r.get(0))
            .unwrap();
        assert_eq!(reason, None, "pre-v6 rows carry NULL");
    }

    #[test]
    fn v7_creates_the_plans_table() {
        let conn = v4_database();
        apply_migration(&conn, 4).unwrap();
        apply_migration(&conn, 5).unwrap();
        apply_migration(&conn, 6).unwrap();
        let v: i64 = conn
            .pragma_query_value(None, "user_version", |r| r.get(0))
            .unwrap();
        assert_eq!(v, 7);
        assert!(conn.prepare("SELECT id, scan_id, root_path, created_at, plan FROM plans").is_ok());
    }

    #[test]
    fn fresh_database_has_the_share_columns() {
        let conn = mem();
        validate_or_init(&conn).unwrap();
        assert!(
            conn.prepare("SELECT private_size, shared_size, clone_id, flags FROM entries")
                .is_ok()
        );
        assert!(conn.prepare("SELECT total_private_size, total_shared_size FROM scans").is_ok());
        assert!(conn.prepare("SELECT failure_reason FROM scans").is_ok());
        assert!(conn.prepare("SELECT plan FROM plans").is_ok());
    }
}
