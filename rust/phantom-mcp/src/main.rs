...
struct Api {
    ...
    /// The host's `clientInfo.name` from `initialize`, header-safe, for the
    /// User-Agent every request carries (phantom-cnr.7). `RefCell` because
    /// the request loop holds `&Api` and `initialize` arrives once.
    client_name: std::cell::RefCell<Option<String>>,
}

/// `phantom-mcp/<version>`, then in parentheses the MCP host that is
/// driving us and the purpose when there is one to declare
/// (`phantom-mcp/1.1.1 (claude-code; +verify)`): the API log then says not
/// just "the MCP server" but which agent host, and whether the scan was
/// asked for or verify's implicit rescan.
fn user_agent_for(client: Option<&str>, purpose: Option<&str>) -> String {
    let mut ua = format!("phantom-mcp/{}", env!("CARGO_PKG_VERSION"));
    let notes: Vec<String> = client
        .map(str::to_string)
        .into_iter()
        .chain(purpose.map(|p| format!("+{p}")))
        .collect();
    if !notes.is_empty() {
        ua.push_str(&format!(" ({})", notes.join("; ")));
    }
    ua
}

/// A host's name goes into a header comment: ASCII, no control characters
/// (a CR/LF would end the header), no parentheses or semicolons (the
/// comment's own grammar), at most 64 characters.
...
impl Api {
    fn from_env() -> Self {
        ...
        Self {
            ...
            key,
            client_name: std::cell::RefCell::new(None),
    ...
    /// Remember the host named in `initialize`'s `clientInfo`.
    fn note_client(&self, client_info: &Value) {
        if let Some(name) = client_info["name"].as_str() {
            let name = sanitize_client_name(name);
            if !name.is_empty() {
                *self.client_name.borrow_mut() = Some(name);
            }
        }
    }

    fn user_agent(&self, purpose: Option<&str>) -> String {
        user_agent_for(self.client_name.borrow().as_deref(), purpose)
    }
    ...
    fn call(
        ...
        self.call_as(method, path, body, None)
    }

    /// [`Self::call`] declaring a purpose in the User-Agent (verify's rescan).
    fn call_as(
        &self,
        method: reqwest::Method,
        path: &str,
        ...
        let mut req = self
            ...
            .header(reqwest::header::USER_AGENT, self.user_agent(purpose));
    ...
    /// Resolve the scan a result tool should read: an explicit `scanId` wins
    /// and carries no note. Otherwise the newest completed scan of ANY root
    /// (phantom_core::pick — the same choice the CLI makes), and the note
    /// names it — `scanId omitted; read scan 5afa7a83 · /Users/ted · 280.4 GB
    /// · 4.09M files · 2 h ago` — plus, when another root has a larger scan
    /// no older than a day, the same warning the CLI prints to stderr. The
    /// note rides on the result as an MCP-envelope field (like the capped
    /// wait's), never on the HTTP wire; the text stays the verbatim body an
    /// agent can parse (phantom-cnr.4).
    fn resolve_scan(&self, args: &Value) -> Result<(String, Option<String>), String> {
        if let Some(id) = args["scanId"].as_str() {
            return Ok((id.to_string(), None));
        }
        let scans: Vec<Scan> = serde_json::from_value(self.call(reqwest::Method::GET, "/scans", None)?)
            .map_err(|e| format!("GET /scans returned a shape this client cannot decode: {e}"))?;
        let chosen = pick::newest_complete(&scans)
            .ok_or_else(|| "no completed scans; run scan_directory first".to_string())?;
        let now = chrono::Utc::now();
        let mut note = format!("scanId omitted; read {}", pick::header_line(chosen, now));
        if let Some(warning) = pick::default_warning(chosen, &scans, now, &|alt| {
            format!("pass scanId \"{}\"", alt.id)
        }) {
            note.push('\n');
            note.push_str(&warning);
        }
        Ok((chosen.id.to_string(), Some(note)))
    }
    ...
    fn find_large_files(&self, id: &str, query: &str) -> Result<Value, String> {
        let resp = self
            ...
            .header("x-api-key", &self.key)
            .header(reqwest::header::USER_AGENT, self.user_agent(None))
            .send()
            .map_err(|e| format!("cannot reach phantom-api at {}: {e}", self.base))?;
        let next = resp
            .headers()
...
/// Attach a defaulted-scan note to an object body; arrays and explicit-id
/// calls pass through untouched, so every explicit call is byte-identical to
/// the HTTP body (the parity gate) and the note is only ever additive.
fn with_note(mut body: Value, note: Option<String>) -> Value {
...
fn handle_tool_call(
    ...
    match name {
        ...
        "plan_reclaim" => {
            let (id, note) = api.resolve_scan(args)?;
            let mut body = json!({});
        ...
        "find_large_files" => {
            ...
            api.find_large_files(&id, &query).map(|v| with_note(v, note))
        }
        "get_space_by_type" => {
            // A bare array body: nothing to hang a note on (structuredContent
            // wraps it as {types}, the text does not). Pass scanId.
            let (id, _note) = api.resolve_scan(args)?;
...
fn handle_request(
    ...
    let resp = match req.method.as_str() {
        ...
        "initialize" => {
            api.note_client(&req.params["clientInfo"]);
            response(
            id,
            json!({
...
fn main() {
    // The only flag this stdio server understands. serverInfo.version on the
    // wire stays the plain Cargo version; this is for a human at a shell asking
    // which build a registration points at (phantom-cnr.6).
    if std::env::args().skip(1).any(|a| a == "--version" || a == "-V") {
        println!("phantom-mcp {}", env!("PHANTOM_VERSION_DISPLAY"));
        return;
    }
...
mod tests {
    ...
    #[test]
    fn user_agent_names_the_binary_the_host_and_the_purpose() {
        let v = env!("CARGO_PKG_VERSION");
        assert_eq!(user_agent_for(None, None), format!("phantom-mcp/{v}"));
        assert_eq!(user_agent_for(Some("claude-code"), None), format!("phantom-mcp/{v} (claude-code)"));
        assert_eq!(user_agent_for(None, Some("verify")), format!("phantom-mcp/{v} (+verify)"));
        assert_eq!(
            user_agent_for(Some("claude-code"), Some("verify")),
            format!("phantom-mcp/{v} (claude-code; +verify)")
        );
    }

    #[test]
    fn a_host_name_cannot_break_the_header() {
        assert_eq!(sanitize_client_name("Claude Code"), "Claude Code");
        // A CR/LF is dropped, not turned into a space: nothing may look like
        // a second header line.
        assert_eq!(sanitize_client_name("Claude (Code); x\r\nX-Evil: 1"), "Claude Code xX-Evil: 1");
        assert_eq!(sanitize_client_name("  spaced  "), "spaced");
        assert_eq!(sanitize_client_name("ünïcödé"), "ncd", "non-ASCII is dropped, the ASCII letters stay");
        assert_eq!(sanitize_client_name(&"a".repeat(100)).len(), 64);
        assert_eq!(sanitize_client_name("()"), "", "nothing left means no comment at all");
    }

    #[test]
    fn initialize_records_the_host_for_later_requests() {
        let api = Api {
        ...
        assert_eq!(api.user_agent(None), user_agent_for(None, None));
        api.note_client(&json!({ "name": "claude-code", "version": "2.0" }));
        assert_eq!(api.user_agent(Some("verify")), user_agent_for(Some("claude-code"), Some("verify")));
        api.note_client(&json!({ "name": "" }));
        assert_eq!(api.user_agent(None), user_agent_for(Some("claude-code"), None), "an empty name changes nothing");
    }
}
