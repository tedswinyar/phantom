...
fn scan_id_property() -> Value {
    json!({
        "type": "string",
        "format": "uuid",
        "description": "Scan UUID. Omit ONLY when a single root has been scanned: the \
            default is the most recent completed scan of ANY root, and a defaulted call carries a \
            `note` naming the scan it read (root, size, age) — and, when another root has a larger \
            scan less than a day old, a warning naming it. Otherwise take the id from list_scans \
            (pick the row whose rootPath is the tree you are working) and pass it on every call. \
            Only the newest 25 scans of each root (100 overall) and a 2 GiB byte budget are kept — a stale \
            id 404s; rescan rather than retrying the id."
    })
}
    ...
    json!([
        ...
        },
        {
            ...
            "description": "Scan a directory tree and record its disk usage. \
                ...
                scan (not idempotent); only the newest 25 per root (100 overall) and a 2 GiB byte budget are kept.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "path": {
            ...
            "inputSchema": {
                ...
                "properties": {
                    "scanA": {
                        "type": "string",
                        "format": "uuid",
                        "description": "The 'before' scan id — the OLDER scan. Only the newest 25 scans of each root and a 2 GiB byte budget are kept (never fewer than a root's last two while it was scanned within 30 days); a pruned baseline is gone (rescanning makes a NEW scan, not the old one), so diff soon after scanning."
                    },
                    "scanB": {
                        "type": "string",
                        "format": "uuid",
        ...
        },
        {
            ...
            "description": "How a root grew across its completed scans, with a \
                ...
                the volume. Only the newest 25 scans of each root and a 2 GiB byte budget are kept \
                (never fewer than a root's last two while it was scanned within 30 days), so the history is the retention window. An unknown root is a not-found error \
                naming list_scans.",
            "inputSchema": {
                "type": "object",
                "properties": {
