...
/// 1.1.1 (phantom-cnr.10): directory rows below 1 MiB are not persisted —
/// EXCEPT every hotspot topPath, because the plan reads a path's private
/// bytes by path and verify reads its before/after aggregates by path. A
/// 100 KiB `target/` is still a cargo-target hotspot; the plan must build
/// (201, not 404) and verify must measure it (beforeBytes non-null).
/// Mutation-proof: drop the topPaths pin in `persist.rs` and the plan 404s
/// ("path … has no row"); drop the ancestor closure and the tree from the
/// root to target/ is broken (`proj` 404s).
#[tokio::test]
async fn plan_creation_succeeds_when_a_hotspot_root_is_under_one_mebibyte() {
    let s = TestServer::start().await;
    let proj = s.workdir().join("smallroot").join("proj");
    std::fs::create_dir_all(proj.join("src")).unwrap();
    std::fs::create_dir_all(proj.join("target").join("debug")).unwrap();
    std::fs::write(proj.join("Cargo.toml"), "[package]\nname = \"p\"\nversion = \"0.1.0\"\n").unwrap();
    std::fs::write(proj.join("Cargo.lock"), "# lock\n").unwrap();
    std::fs::write(proj.join("src").join("main.rs"), "fn main() {}\n").unwrap();
    std::fs::write(proj.join("target").join("debug").join("blob.bin"), vec![7u8; 100 * 1024]).unwrap();
    let root = proj.parent().unwrap().to_path_buf();
    let target = proj.join("target").to_str().unwrap().to_string();

    let scanned = s.scan(&root).await;
    let scan_id = scanned["id"].as_str().unwrap();
    assert!(scanned["totalDiskSize"].as_u64().unwrap() < MIB as u64, "the WHOLE scan is under 1 MiB");

    // The hotspot exists (classification saw the full walk) …
    let hot: serde_json::Value = s.get(&format!("/scans/{scan_id}/hotspots")).await.json().await.unwrap();
    assert_eq!(hot["groups"][0]["ruleId"], "cargo-target", "{hot}");
    assert_eq!(hot["groups"][0]["topPaths"], serde_json::json!([target]));
    // … and its row was kept despite its size, with the chain above it.
    for path in [root.to_str().unwrap(), proj.to_str().unwrap(), target.as_str()] {
        let resp = s.get(&format!("/scans/{scan_id}/entry?path={path}")).await;
        assert_eq!(resp.status(), 200, "{path} must have a row");
    }
    let resp = s.get(&format!("/scans/{scan_id}/entry?path={}", proj.join("src").to_str().unwrap())).await;
    assert_eq!(resp.status(), 404, "src/ is neither big nor a topPath: folded");

    let resp = s.post(&format!("/scans/{scan_id}/plan"), serde_json::json!({})).await;
    assert_eq!(resp.status(), 201, "{}", resp.text().await.unwrap());
    let plan: serde_json::Value = resp.json().await.unwrap();
    assert_eq!(plan["items"][0]["paths"], serde_json::json!([target]));
    let expected = plan["expectedFreedBytes"].as_u64().unwrap();
    assert!(expected > 0 && expected < MIB as u64, "the 100 KiB blob's private bytes: {plan}");

    // Verify reads before/after by path too: the kept row makes the item
    // MEASURED (beforeBytes present), never the unmeasured null branch.
    std::fs::remove_dir_all(proj.join("target")).unwrap();
    let after = s.scan(&root).await;
    let plan_id = plan["planId"].as_str().unwrap();
    let resp = s
        .post(&format!("/plans/{plan_id}/verify"), serde_json::json!({ "afterScanId": after["id"] }))
        .await;
    assert_eq!(resp.status(), 200);
    let v: serde_json::Value = resp.json().await.unwrap();
    assert_eq!(v["items"][0]["beforeBytes"], expected, "{v}");
    assert_eq!(v["items"][0]["afterBytes"], 0);
    assert_eq!(v["items"][0]["actualFreedBytes"], expected);
