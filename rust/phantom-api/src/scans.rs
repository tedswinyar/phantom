...
/// the newest [`crate::config::Retention::DEFAULT_TOTAL`] overall (entries
/// and type totals cascade). Old scans are cheaper to rescan than to keep
/// forever (docs/data-safety.md); the per-root split keeps one root's
/// history from evicting another's (phantom-9tt). Operators override with
/// `PHANTOM_KEEP_SCANS_PER_ROOT` / `PHANTOM_KEEP_SCANS`; `AppState.retention`
...
pub fn finish_scan_with(
    ...
    let (entries, type_totals, hotspots) = match result {
        Ok(outcome) if !state.registry.cancel_requested(id) => {
            ...
            let entries = persist::persistable_entries(&walk, &outcome.shares);
    ...
    match inserted {
        Ok(()) => {
            ...
            let pruned = state.scan_store().prune_retention(retention.per_root, retention.total);
            match pruned {
                Ok(0) => {}
