...
use crate::{AppState, auth, growth, insight, plans, scans};
...
pub fn router(state: AppState) -> Router {
    let authed = Router::new()
        ...
        // Static segment: matchit ranks it above /scans/{id}, so "series"
        // is never parsed as a scan id.
        .route("/scans/series", get(growth::get_series))
        .route(
            "/scans/{id}",
        ...
        .route("/scans/{id}/plan", post(plans::create_plan))
        .route("/plans/{id}", get(plans::get_plan))
        .route("/plans/{id}/script", get(plans::get_plan_script))
        .route("/plans/{id}/verify", post(plans::verify_plan))
        .route("/scans/{id}/explain", get(insight::explain_path))
        .route("/scans/{id}/stale", get(insight::stale_projects))
        .route("/volume", get(insight::volume_status))
