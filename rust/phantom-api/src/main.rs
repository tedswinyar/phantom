...
#[tokio::main]
async fn main() {
    // `phantom-api --version` answers and exits; the stdout announcement line
    // below is a parsed contract and does not change (phantom-cnr.6).
    if std::env::args().skip(1).any(|a| a == "--version" || a == "-V") {
        println!("phantom-api {}", env!("PHANTOM_VERSION_DISPLAY"));
        return;
    }
    // Colour only when stderr is a terminal: the app supervises this
    // process with stderr redirected to ~/Library/Logs/Phantom/phantom-api.log,
    // and a log file full of escape codes is not greppable — every
    // `client=…`/`root=…` field (phantom-cnr.7) must match a plain grep.
