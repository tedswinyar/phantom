...
mod tests {
    ...
    fn mark_failed_sets_a_terminal_visible_state() {
        ...
        let id = scan.id;
        r.register(scan);
        r.mark_failed(id, "worker panicked");
        let (got, _) = r.snapshot(id).unwrap();
        assert_eq!(got.status, ScanStatus::Failed);
        assert!(got.finished_at.is_some());
        assert_eq!(got.failure_reason.as_deref(), Some("worker panicked"));
    }
}
