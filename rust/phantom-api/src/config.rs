...
pub struct Config {
    ...
    /// Scan retention (phantom-9tt, phantom-cnr.3): `PHANTOM_KEEP_SCANS_PER_ROOT`,
    /// `PHANTOM_KEEP_SCANS` and `PHANTOM_DB_BUDGET_BYTES`, else the product
    /// defaults.
    pub retention: Retention,
}

/// What survives a completion's prune: the newest `per_root` of each root
/// (so one root's history is never evicted by another root's scans), then
/// the newest `total` overall, THEN — the binding constraint since 1.1.1
/// (phantom-cnr.3) — a byte budget: while the database's estimated live
/// bytes exceed `budget_bytes`, the globally oldest completed scan of any
/// root still holding more than [`phantom_core::BUDGET_FLOOR_PER_ROOT`]
/// completed scans goes — and, once a root's newest scan is older than
/// [`phantom_core::BUDGET_FLOOR_MAX_AGE_DAYS`], the floor is 0 for it
/// (phantom-ccq). Count alone was the wrong unit: measured
/// 2026-09-16, 25 per root authorised ~10.8 GB for `~` alone. The defaults
/// are a PRODUCT DECISION (25 per root, Ted 2026-09-01; 100 overall,
/// 2026-09-08 with the history feature; 2 GiB, the v1.1.1 plan); the env
/// overrides exist for operators, not as a settings surface.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Retention {
    pub per_root: usize,
    pub total: usize,
    /// `PHANTOM_DB_BUDGET_BYTES`: the estimated live bytes the scan history
    /// may occupy before the oldest scans above the floor are evicted.
...
impl Retention {
    ...
    /// 2 GiB: four home scans at 1.1.0's row density, ~35–50 after the
    /// 1.1.1 directory threshold; ≈ 0.4 % of a 500 GB disk. Fixed rather
    /// than volume-relative because it is explainable in one sentence.
    pub const DEFAULT_BUDGET_BYTES: u64 = 2 * 1024 * 1024 * 1024;

    /// Read `PHANTOM_KEEP_SCANS_PER_ROOT` / `PHANTOM_KEEP_SCANS` /
    /// `PHANTOM_DB_BUDGET_BYTES`; a value that is not a positive integer is
    /// a configuration error, never a silent default (a typo must not
    /// become "keep 0 scans" or "budget 0 bytes").
    pub fn from_env() -> Result<Self, ConfigError> {
        fn read<T: std::str::FromStr + PartialOrd + Default>(
            name: &'static str,
            default: T,
        ...
        let budget_bytes = read("PHANTOM_DB_BUDGET_BYTES", Self::DEFAULT_BUDGET_BYTES)?;
        Ok(Self { per_root, total, budget_bytes })
    }
}
...
impl Default for Retention {
    fn default() -> Self {
        Self {
            per_root: Self::DEFAULT_PER_ROOT,
...
mod tests {
    ...
    fn retention_defaults_and_env_overrides() {
        with_env(&[], || {
            ...
            assert_eq!(
                c.retention,
                Retention { per_root: 25, total: 100, budget_bytes: 2 * 1024 * 1024 * 1024 },
                "the documented defaults (CLAUDE.md, data-safety.md): 25 / 100 / 2 GiB"
            );
        });
        with_env(
            &[
                ("PHANTOM_KEEP_SCANS_PER_ROOT", Some("5")),
                ("PHANTOM_KEEP_SCANS", Some("12")),
                ("PHANTOM_DB_BUDGET_BYTES", Some(" 1048576 ")),
            ],
            || {
                assert_eq!(
                    Config::from_env().unwrap().retention,
                    Retention { per_root: 5, total: 12, budget_bytes: 1_048_576 }
                );
            },
        );
        // Junk, zero, negative, and a total below the per-root cap are
        // errors, never a silent default.
        for (k, v) in [
            ("PHANTOM_KEEP_SCANS", "many"),
            ("PHANTOM_KEEP_SCANS_PER_ROOT", "0"),
            ("PHANTOM_KEEP_SCANS", "-3"),
            ("PHANTOM_DB_BUDGET_BYTES", "2GiB"),
            ("PHANTOM_DB_BUDGET_BYTES", "0"),
            ("PHANTOM_DB_BUDGET_BYTES", "-1"),
        ] {
            with_env(&[(k, Some(v))], || {
