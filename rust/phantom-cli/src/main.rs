...
/// Every request names its sender in the API log (phantom-cnr.7). The
/// implicit rescan of `phantom verify` adds `(+verify)` so the log says
/// who AND why — seven full home rescans nobody typed `phantom scan ~`
/// for were verify's, and only Claude transcripts could show it.
const USER_AGENT: &str = concat!("phantom-cli/", env!("CARGO_PKG_VERSION"));
const USER_AGENT_VERIFY: &str = concat!("phantom-cli/", env!("CARGO_PKG_VERSION"), " (+verify)");

#[derive(Parser)]
#[command(
    name = "phantom",
    // "1.1.1" on a release build; "1.1.1 (v1.1.0-14-g3896576-dirty)" on any
    // other, so a dogfood binary can never pass for the release
    // (rust/build-support/git-describe.rs, phantom-cnr.6).
    version = env!("PHANTOM_VERSION_DISPLAY"),
...
enum Command {
    ...
    Top {
        /// Scan id (default: the most recent completed scan of ANY root — check the header line)
        #[arg(long)]
        scan: Option<Uuid>,
        /// The scan id, positionally — the same as --scan (a UUID cannot be mistaken for anything else)
        #[arg(value_name = "SCAN_ID", conflicts_with = "scan")]
        scan_id: Option<Uuid>,
...
impl Client {
    ...
    /// Every request carries the key and the User-Agent; `user_agent` is
    /// [`USER_AGENT`] except where a caller has a purpose to declare.
    fn build(&self, method: reqwest::Method, path: &str, user_agent: &str) -> reqwest::blocking::RequestBuilder {
        self.http
    ...
    fn request(
        ...
        self.request_as(method, path, body, USER_AGENT)
    }

    /// [`Self::request`] under a specific User-Agent (verify's rescan).
    fn request_as(
        &self,
        method: reqwest::Method,
        path: &str,
        ...
        let mut req = self.build(method, path, user_agent);
    ...
    fn request_no_content(&self, method: reqwest::Method, path: &str) {
        let req = self.build(method, path, USER_AGENT);
        let resp = self.dispatch(req);
    ...
    /// The scan a result command reads, plus its row for the header line.
    /// An explicit id (positional or --scan) is fetched as given. Otherwise
    /// the newest completed scan of ANY root (phantom_core::pick — the same
    /// choice the MCP server makes), and when another root has a larger scan
    /// no older than a day, a warning on stderr names it and the remedy.
    /// stderr only, so --json output stays the API body (phantom-cnr.4).
    fn resolve_scan(&self, explicit: Option<Uuid>) -> (String, Scan) {
        match explicit {
            Some(id) => {
                let scan: Scan = decode(
                    self.request(reqwest::Method::GET, &format!("/scans/{id}"), None),
                    "scan",
                );
                (id.to_string(), scan)
            }
            None => {
                let scans: Vec<Scan> =
                    decode(self.request(reqwest::Method::GET, "/scans", None), "scan list");
                let Some(chosen) = pick::newest_complete(&scans) else {
                    eprintln!("phantom: no completed scans; run `phantom scan <path>` first");
                    std::process::exit(EXIT_NOT_FOUND);
                };
                if let Some(warning) = pick::default_warning(chosen, &scans, chrono::Utc::now(), &|alt| {
                    format!("pass --scan {}", alt.id)
                }) {
                    eprintln!("phantom: {warning}");
                }
                (chosen.id.to_string(), chosen.clone())
            }
        }
    }
...
/// The first line of every human-format result view: which scan this is
/// (phantom_core::pick::header_line — the MCP note carries the same text).
fn scan_header(scan: &Scan) -> String {
    pick::header_line(scan, chrono::Utc::now())
}
...
fn main() {
    ...
    match cli.command {
        Command::Plan {
            ...
            let (id, picked) = client.resolve_scan(scan_id.or(scan));
        ...
        Command::Verify { plan_id, after, json } => {
            let after_id = match after {
                ...
                None => {
                    ...
                    // The rescan announces itself as verify's in the API log
                    // (phantom-cnr.7), and the human line names the scan so
                    // the log, `phantom scans list` and this terminal agree.
                    let started = client.request_as(
                        reqwest::Method::POST,
                        "/scans",
                    ...
                    if !json {
                        eprintln!(
                            "phantom: rescanning {root} as scan {id} to verify plan {plan_id} \
                             (a full scan of the root); pass --after <scanId> to reuse a \
        ...
        Command::Explain { path, scan, scan_id, json } => {
            let (id, picked) = client.resolve_scan(scan_id.or(scan));
            let v = client.request(
                reqwest::Method::GET,
        ...
        Command::Stale { scan, scan_id, older, json } => {
            let (id, picked) = client.resolve_scan(scan_id.or(scan));
            let query = older
        ...
        Command::Scans { command } => match command {
            ...
            ScansCommand::Delete { id } => {
                ...
                // The API compacts the database on the same request (1.1.1,
                // phantom-cnr.2), so the bytes are back when this prints.
                // How many is not on the wire yet (v1.2 storage surface); the
                // API log has the number.
                println!("deleted {id}; the database was compacted (bytes returned to the filesystem — see the API log)");
            }
        },
        Command::Top {
            scan,
        ...
        Command::Types { scan, scan_id, json } => {
            let (id, picked) = client.resolve_scan(scan_id.or(scan));
            let v = client.request(
                reqwest::Method::GET,
        ...
        Command::Hotspots { scan, scan_id, json } => {
            let (id, picked) = client.resolve_scan(scan_id.or(scan));
            let v = client.request(
                reqwest::Method::GET,
...
mod tests {
    #[test]
    fn user_agent_names_the_binary_its_version_and_verify_s_purpose() {
        assert_eq!(super::USER_AGENT, format!("phantom-cli/{}", env!("CARGO_PKG_VERSION")));
        assert_eq!(super::USER_AGENT_VERIFY, format!("{} (+verify)", super::USER_AGENT));
    }
